...
class UserSettingsManager {
        ...
        fetch(this.resetToDefaultsUrl, {
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    this.showAlert(
                        'danger',
                        'Error resetting settings: ' + (data.message || data.error || 'Unknown error')
                    );
                }
            })
            .catch((error) => {
        ...
        fetch(this.userSettingsUrl, {
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    this.showAlert(
                        'danger',
                        'Error saving settings: ' + (data.message || data.error || 'Unknown error')
                    );
                }
            })
            .catch((error) => {
    ...
    initTemplatePreview() {
        ...
        // Handle template selection to show preview
        templateSelector.addEventListener('change', (e) => {
            ...
            const folderPattern = selectedOption.dataset.folder || '';
            const filenamePattern = selectedOption.dataset.filename || '';
            const description = selectedOption.dataset.description || '';

            // Update preview displays
            document.getElementById('folder-pattern-preview').textContent = folderPattern;
            document.getElementById('filename-pattern-preview').textContent = filenamePattern;

            // Generate a sample path for the full preview
        ...
        const config = window.userSettingsConfig || {};
        if (config.defaultTemplateKey) {
            templateSelector.value = config.defaultTemplateKey;
            // Trigger change event to show preview
            templateSelector.dispatchEvent(new Event('change'));
        }

        // Update "Edit Templates" button to pass selected template
        if (editTemplatesBtn) {
            editTemplatesBtn.addEventListener('click', (e) => {
                const selectedValue = templateSelector.value;
                if (selectedValue) {
                    e.preventDefault();
                    const baseUrl = editTemplatesBtn.href;
                    window.location.href = `${baseUrl}?template=${encodeURIComponent(selectedValue)}`;
                }
                // If no template selected, just go to the page normally
            });
        }

        // Handle save default template button
        const saveDefaultBtn = document.getElementById('save-default-template-btn');
        if (saveDefaultBtn) {
            saveDefaultBtn.addEventListener('click', () => this.saveDefaultTemplate());
        }

        // Handle pattern example clicks
        const exampleItems = document.querySelectorAll('.example-clickable');
        exampleItems.forEach((item) => {
            item.addEventListener('click', (e) => {
                const folderPattern = e.currentTarget.dataset.folder;
                const filenamePattern = e.currentTarget.dataset.filename;

                // Find matching template
                const options = templateSelector.options;
                let matchFound = false;

                for (let i = 0; i < options.length; i++) {
                    const option = options[i];
                    if (option.dataset.folder === folderPattern && option.dataset.filename === filenamePattern) {
                        templateSelector.value = option.value;
                        templateSelector.dispatchEvent(new Event('change'));
                        matchFound = true;

                        // Scroll to template selector and highlight it briefly
                        templateSelector.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        templateSelector.classList.add('border-success', 'border-3');
                        setTimeout(() => {
                            templateSelector.classList.remove('border-success', 'border-3');
                        }, 2000);
                ...
                if (!matchFound) {
                    // If no exact match, clear selection and show preview with the patterns
                    ...
                    templateSelector.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    templateSelector.classList.add('border-warning', 'border-3');
                    setTimeout(() => {
                        templateSelector.classList.remove('border-warning', 'border-3');
                    }, 2000);
                }
            });
        });
    ...
    async saveDefaultTemplate() {
        ...
        const templateKey = selectedOption.value;
        const folderPattern = selectedOption.dataset.folder;
        const filenamePattern = selectedOption.dataset.filename;
        ...
        try {
            ...
            formData.append('folder_pattern', folderPattern);
            formData.append('filename_pattern', filenamePattern);
            formData.append('csrfmiddlewaretoken', this.csrfToken);

            const response = await fetch('/settings/save-default-template/', {
                method: 'POST',
                body: formData,
            });
            ...
            if (data.success) {
                // Show success message
                const btn = document.getElementById('save-default-template-btn');
    ...
    /**
     * Generate a sample file path from patterns
     */
    generateSamplePath(folderPattern, filenamePattern) {
        ...
        Object.entries(replacements).forEach(([token, value]) => {
            folder = folder.replace(new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), value);
