...
window.scanningDashboardInline = {
            ...
            if (progress.needs_scan) {
                ...
                if (progressPercent) {
                    progressPercent.innerHTML =
                        '<span class="badge status-success"><i class="fas fa-check me-1"></i>Complete</span>';
                }
            }
        }
    },
                        ...
                        .then((data) => {
                            if (data.status === 'success') {
                                ...
                                scanAllBtn.innerHTML =
                                    '<i class="fas fa-check-circle me-2 text-white"></i>Scans Started!';
                                scanAllBtn.classList.remove('btn-success');
                                scanAllBtn.classList.add('btn-success', 'opacity-75');

                                // Show notification
                                window.scanningDashboardInline.showNotification(
                                    'success',
                                    `Started scans for ${data.folder_count} folder(s)`,
                                    data.message
                                );
                                ...
                                window.scanningDashboardInline.showNotification(
                                    'error',
                                    'Scan Failed',
                                    data.message || 'Failed to start scans'
                                );
    ...
    showNotification(type, title, message) {
        ...
        notification.className = `alert alert-${type === 'success' ? 'success' : 'danger'} alert-dismissible fade show position-fixed app-toast-fixed`;
...
window.updateProgressContainer = (container, progress) =>
    window.scanningDashboardInline.updateProgressContainer(container, progress);
window.showProgressError = (container, message) => window.scanningDashboardInline.showProgressError(container, message);
window.showNotification = (type, title, message) =>
    window.scanningDashboardInline.showNotification(type, title, message);
