...
from ..models import Book, FinalMetadata
...
class EbooksMainView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Count ebooks from scan folders designated as 'ebooks' or 'mixed'
        try:
            ebooks_count = Book.objects.filter(
                scan_folder__content_type__in=['ebooks', 'mixed'],
...
def ebooks_ajax_list(request):
    ...
    try:
        # Get ebooks from scan folders designated as 'ebooks' or 'mixed'
...
class AudiobooksMainView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Count audiobooks from scan folders designated as 'audiobooks' or 'mixed'
        audiobooks_count = Book.objects.filter(
            scan_folder__content_type__in=['audiobooks', 'mixed'],
            scan_folder__is_active=True
        ).count()
...
def audiobooks_ajax_list(request):
    ...
    try:
        # Get audiobooks from scan folders designated as 'audiobooks' or 'mixed'
