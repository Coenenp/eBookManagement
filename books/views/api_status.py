...
class APIStatusView(LoginRequiredMixin, BookNavigationMixin, ListView):
    ...
    paginate_by = PAGINATION["api_status"]

    def get_queryset(self):
        """Get books with API completeness tracking"""
        queryset = (
            Book.objects.filter(api_completeness__isnull=False)
            .select_related("api_completeness", "finalmetadata")
            .order_by("api_completeness__scan_priority", "-api_completeness__metadata_completeness")
        )
        ...
        if missing_source:
            queryset = queryset.filter(api_completeness__missing_sources__contains=[missing_source])
        ...
        if completeness_range:
            ...
            if completeness_range in ranges:
                ...
                queryset = queryset.filter(api_completeness__metadata_completeness__gte=min_val, api_completeness__metadata_completeness__lt=max_val)
...
def retry_all_priority(request, priority):
    ...
    try:
        books = Book.objects.filter(api_completeness__scan_priority=priority, api_completeness__needs_external_scan=True).select_related("api_completeness")[
            :100
        ...
        return JsonResponse(
            {"success": True, "message": f"Processed {len(books)} books. {success_count} had successful API calls.", "processed": len(books), "successful": success_count}
        )
...
def resume_failed_api_calls(request, session_id):
    ...
    try:
        ...
        messages.success(request, f"Processed {success_count + failed_count} books. " f"Success: {success_count}, Failed: {failed_count}")
...
def api_health_status(request):
    ...
    return JsonResponse({"apis": apis, "timestamp": api_status.get("timestamp", None)})
