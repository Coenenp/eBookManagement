...
def get_issue_statistics() -> Dict[str, int]:
    ...
    return {
        ...
        "series_without_numbers": BookSeries.objects.filter(Q(is_active=True) & (Q(series_number="") | Q(series_number__isnull=True))).count(),
    }
...
def get_recent_activity(days: int = 7) -> Dict[str, int]:
    ...
    return {
        "recently_added": Book.objects.filter(
