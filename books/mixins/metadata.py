...
class SourceConfidenceMixin(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, help_text="Uncheck to hide this metadata without deleting it."
    )
...
class MetadataContextMixin:
    ...
    def get_metadata_context(self, book):
        ...
        context["all_titles"] = book.titles.filter(is_active=True).order_by(
            "-confidence"
        )
        context["all_authors"] = book.author_relationships.filter(
            is_active=True
        ).order_by("-confidence", "-is_main_author")
        context["all_genres"] = book.genre_relationships.filter(
            is_active=True
        ).order_by("-confidence")
        context["all_series"] = book.series_relationships.filter(
            is_active=True
        ).order_by("-confidence")

        # If no series relationships but there is series info in final metadata
        if not context["all_series"].exists():
            series_number_metadata = book.metadata.filter(
                is_active=True, field_name="series_number"
            ).first()
            try:
                ...
                if series_number_metadata or (
                    final_metadata and final_metadata.final_series
                ):
                    context["has_series_number_only"] = True
                    context["series_number_metadata"] = series_number_metadata
                    context["final_series_name"] = (
                        getattr(final_metadata, "final_series", "")
                        if final_metadata
                        else ""
                    )
                    context["final_series_number"] = (
                        getattr(final_metadata, "final_series_number", "")
                        if final_metadata
                        else ""
                    )
            except Exception:
        ...
        context["all_publishers"] = book.publisher_relationships.filter(
            is_active=True
        ).order_by("-confidence")
        context["all_covers"] = book.covers.filter(is_active=True).order_by(
            "-confidence", "-is_high_resolution"
        )

        # Current genres for checkboxes
        current_genres = list(
            book.genre_relationships.filter(is_active=True).values_list(
        ...
        context["has_conflicts"] = (
            context["all_titles"].count() > 1
            or context["all_authors"].count() > 1
            or context["all_series"].count() > 1
            or context["all_publishers"].count() > 1
        )
    ...
    def get_metadata_fields_context(self, book):
        ...
        return {
            "series_number_metadata": book.metadata.filter(
                field_name="series_number", is_active=True
            ).order_by("-confidence"),
            "description_metadata": book.metadata.filter(
                field_name="description", is_active=True
            ).order_by("-confidence"),
            "isbn_metadata": book.metadata.filter(
                field_name="isbn", is_active=True
            ).order_by("-confidence"),
            "year_metadata": book.metadata.filter(
                field_name="publication_year", is_active=True
            ).order_by("-confidence"),
            "language_metadata": book.metadata.filter(
                field_name="language", is_active=True
            ).order_by("-confidence"),
...
class BookListContextMixin:
    ...
    def get_list_statistics_context(self):
        ...
        all_languages = (
            apps.get_model("books", "FinalMetadata")
            .objects.values_list("language", flat=True)
            .distinct()
        )
        valid_language_codes = LanguageManager.get_valid_codes()
        used_languages = [
            lang for lang in all_languages if lang in valid_language_codes
        ]

        # Create language choices for template
        lang_dict = LanguageManager.get_language_dict()
        languages = [
            (code, lang_dict[code]) for code in used_languages if code in lang_dict
        ]
    ...
    def get_filter_context(self):
        ...
        filter_context["corrupted_count"] = Book.objects.filter(
            is_corrupted=True
        ).count()
