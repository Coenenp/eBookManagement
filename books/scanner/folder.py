...
def discover_books_in_folder(directory, ebook_extensions=None, cover_extensions=None):
    ...
    if ebook_extensions is None:
        ...
        all_formats = set([f".{fmt}" for fmt in EBOOK_FORMATS] + [f".{fmt}" for fmt in COMIC_FORMATS] + [f".{fmt}" for fmt in AUDIOBOOK_FORMATS])
...
def _cleanup_missing_books(directory, scan_folder, found_files):
    ...
    existing_files = BookFile.objects.filter(book__scan_folder=scan_folder, file_path__startswith=directory).select_related("book")
...
def _create_placeholder_book(file_path, scan_folder):
    try:
        book, created = _get_or_create_book_by_path(file_path=file_path, scan_folder=scan_folder, file_format="placeholder")
        if created:
