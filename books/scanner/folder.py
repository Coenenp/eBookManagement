...
from books.scanner.extractors import comic, epub, mobi, opf, pdf
...
def _get_or_create_book_by_path(
    file_path, scan_folder, file_format=None, file_size=None, content_type="ebook"
):
    """
    ...
    existing_file = (
        BookFile.objects.filter(file_path=file_path).select_related("book").first()
    )
    if existing_file:
        ...
        if book.deleted_at is not None:
            logger.info(
                f"[REACTIVATE] Reactivating previously deleted book: {file_path}"
            )
    ...
    with transaction.atomic():
        ...
        BookFile.objects.create(
            ...
            file_size=file_size
            or (os.path.getsize(file_path) if os.path.exists(file_path) else None),
        )
...
def scan_directory(
    ...
    ebook_files, cover_files, opf_files = _collect_files(
        directory, ebook_extensions, cover_extensions
    )
    ...
    if resume_from:
        ...
        try:
            ...
            if resume_index > 0:
                logger.info(
                    f"Resuming file processing from file #{resume_index + 1} out of {len(ebook_files)}"
                )
                ebook_files = ebook_files[resume_index:]
            else:
                logger.info(
                    "Resume file not found in current directory, starting from beginning"
                )
        except Exception as e:
            logger.warning(
                f"Error processing resume point: {e}. Starting from beginning."
            )
    ...
    try:
        ...
        if scan_folder.content_type in [
            ...
            logger.info(
                f"Using content-type specific processing for {scan_folder.content_type}"
            )
            process_files_by_type(
                ebook_files, scan_folder, cover_files, opf_files, rescan
            )
            ...
            update_scan_progress(
                scan_status,
                scan_status.processed_files,
                total_files,
                "Content-type processing complete",
            )
        else:
            # Use original individual file processing for ebooks
            _process_files_individually(
                ebook_files,
                scan_folder,
                cover_files,
                opf_files,
...
def _should_use_content_type_processing(ebook_files):
    ...
    comic_count = sum(
        ...
        if any(f.lower().endswith(f".{ext}") for ext in COMIC_FORMATS)
    )
    audio_count = sum(
        1
        for f in ebook_files
        if any(f.lower().endswith(f".{ext}") for ext in AUDIOBOOK_FORMATS)
    )
...
def _process_files_individually(
    ebook_files, scan_folder, cover_files, opf_files, rescan, scan_status, total_files
):
    ...
    for i, ebook_path in enumerate(ebook_files, 1):
        ...
        update_scan_progress(
            scan_status, scan_status.processed_files, total_files, Path(ebook_path).name
        )
...
def _find_incomplete_metadata_books(directory, scan_folder):
    ...
    incomplete_books = (
        Book.objects.filter(scan_folder=scan_folder, file_path__startswith=directory)
        .exclude(
    ...
    logger.info(
        f"Found {incomplete_books.count()} books needing metadata completion in {directory}"
    )
...
def _complete_metadata_for_books(books, scan_status):
    ...
    for i, book in enumerate(books, 1):
        try:
            logger.info(
                f"[METADATA COMPLETION] Processing book {book.id}: {book.primary_file.file_path if book.primary_file else 'No file'}"
            )
            ...
            logger.info(
                f"[METADATA and COVER CANDIDATES QUERY] Path: {book.primary_file.file_path if book.primary_file else 'No file'}"
            )
            query_metadata_and_covers(book)

            try:
                logger.info(
                    f"[FINAL METADATA RESOLVE] Path: {book.primary_file.file_path if book.primary_file else 'No file'}"
                )
                resolve_final_metadata(book)
                logger.info(f"Completed metadata for book {book.id}")
            except Exception as e:
                logger.error(
                    f"Final metadata resolution failed for {book.primary_file.file_path if book.primary_file else 'No file'}: {str(e)}"
                )
        ...
        # Update progress
        scan_status.message = (
            f"Completing metadata for book {book.id} ({i}/{total_incomplete})"
        )
        scan_status.save()
...
def _process_book(file_path, scan_folder, cover_files, opf_files, rescan=False):
    ...
    logger.info(
        f"[INTERNAL METADATA PARSE] Path: {book.primary_file.file_path if book.primary_file else 'No file'}"
    )
    _extract_internal_metadata(book)

    # Skip ISBN scanning for comic books (comics don't typically have ISBNs)
    is_comic = (
        book.primary_file.file_format.lower() in COMIC_FORMATS
        if book.primary_file
        else False
    )

    if not is_comic:
        logger.info(
            f"[CONTENT ISBN SCAN] Path: {book.primary_file.file_path if book.primary_file else 'No file'}"
        )
        try:
    ...
    logger.info(
        f"[OPF PARSE] Path: {book.primary_file.file_path if book.primary_file else 'No file'}"
    )
    ...
    if not is_comic:
        ...
        logger.info(
            f"[SKIPPING EXTERNAL QUERIES] Comic book detected: {book.primary_file.file_path if book.primary_file else 'No file'}"
        )
...
def _extract_filename_metadata(book):
    ...
    if is_comic:
        ...
        parsed = parse_path_metadata(
            book.primary_file.file_path if book.primary_file else ""
        )
    ...
    logger.info(
        f"[FILENAME PARSE] Parsed series: {parsed.get('series')} (#{parsed.get('series_number')})"
    )
...
def _extract_internal_metadata(book):
    ...
    try:
        if fmt == "epub":
            ...
            logger.info(
                f"[SKIPPED] Comic format {fmt.upper()} not yet supported for metadata extraction: {book.file_path}"
            )
            return
        ...
        logger.warning(
            f"[CORRUPT FILE DETECTED] {fmt.upper()} extract failed for {book.file_path}: {e}"
        )
        book.is_corrupted = True
        book.save()
...
def _cleanup_missing_books(directory, scan_folder, found_files):
    ...
    existing_files = BookFile.objects.filter(
        book__scan_folder=scan_folder, file_path__startswith=directory
    ).select_related("book")
    ...
    for book_file in existing_files:
        ...
        if not file_exists and not file_was_found:
            ...
            if book.deleted_at is None:
                logger.info(
                    f"[CLEANUP] Marking book as deleted (file not found): {book_file.file_path}"
                )
                book.soft_delete()
                marked_as_deleted += 1
        else:
            ...
            if book.deleted_at is not None:
                logger.info(
                    f"[CLEANUP] Reactivating book (file found again): {book_file.file_path}"
                )
                book.deleted_at = None
                book.is_available = True
                book.last_scanned = timezone.now()
    ...
    if marked_as_deleted > 0:
        logger.info(
            f"[CLEANUP] Marked {marked_as_deleted} books as deleted (files not found)"
        )
    if reactivated > 0:
...
def _create_placeholder_book(file_path, scan_folder):
    try:
        book, created = _get_or_create_book_by_path(
            file_path=file_path, scan_folder=scan_folder, file_format="placeholder"
        )
        if created:
...
def query_external_metadata(book):
    ...
    try:
        ...
        if is_comic:
            logger.info(
                f"[EXTERNAL METADATA] Skipping external queries for comic: {book.file_path}"
            )
            return True
