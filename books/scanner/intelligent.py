...
from books.models import APIAccessLog, ScanSession, BookAPICompleteness
...
class IntelligentAPIScanner:
    ...
    def scan_book_with_intelligence(self, book: Book, force_all_apis: bool = False) -> Dict[str, any]:
        ...
        try:
            ...
            for api_name in apis_to_attempt:
                ...
                if result['success']:
                    ...
                    if result.get('should_retry', False):
                        ...
                        self.session.add_book_to_resume_queue(
                            book.id, [api_name]
                        )
            ...
            self.session.save(update_fields=[
                'processed_books', 'books_with_external_data'
            ])
    ...
    def resume_interrupted_scans(self) -> Dict[str, any]:
        ...
        try:
            ...
            resumable_sessions = ScanSession.objects.filter(
                can_resume=True,
                is_active=True
            ).order_by('-created_at')[:5]  # Limit to recent sessions
    ...
    def get_scanning_recommendations(self, scan_folder_id: Optional[int] = None) -> Dict[str, any]:
        ...
        try:
            ...
            books_needing_retry = APIAccessLog.objects.filter(
                should_retry=True,
                can_retry_now=True
    ...
    def complete_session(self):
        ...
        if self.session:
            ...
            self.session.save(update_fields=[
                'is_active', 'completed_at', 'can_resume'
            ])
