...
def _detect_issue_type(filename):
    ...
    if re.search(
        r"\btpb\b|\btrade\b.*\bpaperback\b|\bgraphic\b.*\bnovel\b|\bhc\b|\bhardcover\b",
        filename_lower,
    ):
        return "collection"
...
def _extract_special_identifiers(filename):
    ...
    if re.search(
        r"\bvariant\b|\balt\b.*\bcover\b|\bcover\b.*\b[abc]\b|\bincentive\b.*\bcover\b|\bsketch\b.*\bcover\b",
        filename_lower,
    ):
