...
class DuplicateHandlingTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create data sources
        self.initial_source, _ = DataSource.objects.get_or_create(name=DataSource.INITIAL_SCAN, defaults={"trust_level": 0.2})

        self.epub_source, _ = DataSource.objects.get_or_create(name=DataSource.EPUB_INTERNAL, defaults={"trust_level": 0.7})
    ...
    def test_same_title_different_authors_separate_entries(self):
        ...
        book1 = create_test_book_with_file(file_path="/test/path/common_title1.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)

        book2 = create_test_book_with_file(file_path="/test/path/common_title2.epub", file_format="epub", file_size=1048576, scan_folder=self.scan_folder)

        # Create final metadata with same title, different authors
        FinalMetadata.objects.create(book=book1, final_title="The Common Title", final_author="John Smith", is_reviewed=False)

        FinalMetadata.objects.create(book=book2, final_title="The Common Title", final_author="Jane Doe", is_reviewed=False)

        # Create author relationships
        BookAuthor.objects.create(book=book1, author=author1, source=self.epub_source, confidence=0.9, is_main_author=True)

        BookAuthor.objects.create(book=book2, author=author2, source=self.epub_source, confidence=0.9, is_main_author=True)
    ...
    def test_identical_file_path_prevents_duplicates(self):
        ...
        book1 = create_test_book_with_file(file_path=file_path, file_format="epub", file_size=1024000, scan_folder=self.scan_folder)
        ...
        with patch.object(Book, "get_or_create_by_path") as mock_get_or_create:
            ...
            result_book, created = Book.get_or_create_by_path(
                file_path=file_path, defaults={"file_format": "epub", "file_size": 2048000, "scan_folder": self.scan_folder}  # Different size
            )
    ...
    def test_same_title_same_author_different_editions(self):
        ...
        book1 = create_test_book_with_file(file_path="/test/path/the_shining_first_edition.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)

        book2 = create_test_book_with_file(file_path="/test/path/the_shining_revised_edition.epub", file_format="epub", file_size=1048576, scan_folder=self.scan_folder)

        # Same title and author, different ISBNs/editions
        FinalMetadata.objects.create(book=book1, final_title="The Shining", final_author="Stephen King", isbn="978-1111111111", publication_year=1977)  # Different ISBNs

        FinalMetadata.objects.create(
            book=book2, final_title="The Shining", final_author="Stephen King", isbn="978-2222222222", publication_year=2013  # Different ISBNs  # Different publication years
        )

        # Should maintain as separate entries (different editions)
        books_count = FinalMetadata.objects.filter(final_title="The Shining", final_author="Stephen King").count()
    ...
    def test_case_insensitive_title_comparison_for_duplicates(self):
        ...
        book1 = create_test_book_with_file(file_path="/test/path/book1.epub", file_format="epub", scan_folder=self.scan_folder)

        book2 = create_test_book_with_file(file_path="/test/path/book2.epub", file_format="epub", scan_folder=self.scan_folder)

        # Create metadata with case variations
        FinalMetadata.objects.create(book=book1, final_title="the test book", final_author="Test Author")  # lowercase

        FinalMetadata.objects.create(book=book2, final_title="The Test Book", final_author="Test Author")  # title case

        # Query should find both as potential duplicates if doing case-insensitive comparison
        lowercase_matches = FinalMetadata.objects.filter(final_title__iexact="the test book", final_author="Test Author").count()
    ...
    def test_file_hash_duplicate_detection(self):
        ...
        book1 = create_test_book_with_file(file_path="/test/path/original.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)

        book2 = create_test_book_with_file(file_path="/test/path/copy.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)  # Different path  # Same size
    ...
    def test_author_name_normalization_for_duplicates(self):
        ...
        FinalMetadata.objects.create(book=book1, final_title="Sample Book", final_author="J.K. Rowling")  # With periods

        FinalMetadata.objects.create(book=book2, final_title="Sample Book", final_author="JK Rowling")  # Without periods

        # Both should be treated as potentially different authors
        # unless normalization logic exists
        author_variations = FinalMetadata.objects.filter(final_title="Sample Book").values_list("final_author", flat=True)
    ...
    def test_isbn_based_duplicate_detection(self):
        ...
        FinalMetadata.objects.create(book=book1, final_title="Title Version A", final_author="Author A", isbn=isbn)

        FinalMetadata.objects.create(book=book2, final_title="Title Version B", final_author="Author B", isbn=isbn)
    ...
    def test_series_differentiation_same_title(self):
        ...
        book1 = create_test_book_with_file(file_path="/test/path/dragons1.epub", file_format="epub", scan_folder=self.scan_folder)

        book2 = create_test_book_with_file(file_path="/test/path/dragons2.epub", file_format="epub", scan_folder=self.scan_folder)

        # Same title, same author, different series
        FinalMetadata.objects.create(book=book1, final_title="Dragons", final_author="Fantasy Author", final_series="Epic Fantasy Series")

        FinalMetadata.objects.create(book=book2, final_title="Dragons", final_author="Fantasy Author", final_series="Modern Fantasy Series")

        # Should remain separate due to different series
        dragons_books = FinalMetadata.objects.filter(final_title="Dragons", final_author="Fantasy Author")
