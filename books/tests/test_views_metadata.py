...
from books.models import Author, BookAuthor, BookCover, BookGenre, BookMetadata, BookPublisher, BookSeries, BookTitle, DataSource, FinalMetadata, Genre, Publisher, Series
...
class BookMetadataViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test book
        self.book = create_test_book_with_file(file_path="/test/book1.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
    ...
    def create_metadata_entries(self):
        ...
        # Book titles
        BookTitle.objects.create(book=self.book, title="Test Book", confidence=0.9, is_active=True, source=self.manual_source)
        BookTitle.objects.create(book=self.book, title="Alternative Title", confidence=0.7, is_active=True, source=self.epub_source)

        # Book authors
        BookAuthor.objects.create(book=self.book, author=self.author, confidence=0.9, is_main_author=True, is_active=True, source=self.manual_source)

        # Book series
        BookSeries.objects.create(book=self.book, series=self.series, series_number="1", confidence=0.8, is_active=True, source=self.epub_source)

        # Book publisher
        BookPublisher.objects.create(book=self.book, publisher=self.publisher, confidence=0.7, is_active=True, source=self.epub_source)

        # Book genre
        BookGenre.objects.create(book=self.book, genre=self.genre, confidence=0.8, is_active=True, source=self.epub_source)

        # Book cover
        BookCover.objects.create(book=self.book, cover_path="/media/covers/test.jpg", confidence=0.9, is_high_resolution=True, is_active=True, source=self.manual_source)
        ...
        for field_name, value in [
            ...
            BookMetadata.objects.create(book=self.book, field_name=field_name, field_value=value, confidence=0.8, is_active=True, source=self.epub_source)
    ...
    def test_navigation_context(self):
        ...
        book2 = create_test_book_with_file(file_path="/test/book2.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
        book3 = create_test_book_with_file(file_path="/test/book3.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
...
class BookMetadataUpdateViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create final metadata
        self.final_metadata = FinalMetadata.objects.create(
            book=self.book, final_title="Original Title", final_author="Original Author", is_reviewed=True  # Set to True to prevent auto-sync during manual updates
        )

    def test_metadata_update_requires_login(self):
        """Test that metadata update requires authentication"""
        self.client.logout()
        response = self.client.post(reverse("books:book_metadata_update", kwargs={"pk": self.book.pk}), {"final_title": "New Title"})
        self.assertEqual(response.status_code, 302)
        self.assertIn("login", response.url)

    def test_metadata_update_requires_post(self):
        """Test that metadata update only accepts POST requests"""
        response = self.client.get(reverse("books:book_metadata_update", kwargs={"pk": self.book.pk}))
        # View handles GET with redirect instead of 405
        self.assertEqual(response.status_code, 302)

    def test_basic_metadata_update(self):
        """Test basic metadata field updates"""
        update_data = {"final_title": "Updated Title", "final_author": "Updated Author", "final_series": "Updated Series", "final_publisher": "Updated Publisher"}
    ...
    def test_series_number_with_series(self):
        ...
        update_data = {"final_title": self.final_metadata.final_title, "final_series": "Test Series", "final_series_number": "2"}  # Required field
    ...
    def test_series_number_without_series_warning(self):
        ...
        update_data = {
            "final_title": self.final_metadata.final_title,  # Required field
    ...
    def test_cover_field_processing(self):
        ...
        # Test updating is_reviewed flag only (partial update)
        update_data = {"final_title": self.final_metadata.final_title, "is_reviewed": "on"}  # Required field  # Checkbox value
    ...
    def test_numeric_fields_processing(self):
        """Test numeric fields like publication year"""
        update_data = {"final_title": self.final_metadata.final_title, "publication_year": "2023", "isbn": "9781234567890"}  # Required field
        ...
        self.final_metadata.refresh_from_db()
        # Check that numeric field processing worked
    ...
    def test_genre_fields_processing(self):
        ...
        # Test manual_genres field (comma-separated genres)
        update_data = {"final_title": self.final_metadata.final_title, "manual_genres": "Science Fiction, Fantasy"}  # Required field

        response = self.client.post(reverse("books:book_metadata_update", kwargs={"pk": self.book.pk}), update_data)

        self.assertEqual(response.status_code, 302)

        # Check that form submission worked (genre processing happens in clean method)
        self.final_metadata.refresh_from_db()
    ...
    def test_metadata_fields_processing(self):
        ...
        update_data = {
            ...
            "isbn": "9781234567890",
            "publication_year": "2023",
            "language": "en",  # Use language code directly
        ...
        self.assertEqual(self.final_metadata.language, "en")
    ...
    def test_review_status_update(self):
        ...
        # Mark as reviewed
        update_data = {"final_title": self.final_metadata.final_title, "is_reviewed": "on"}  # Required field
        ...
        # Unmark as reviewed (no checkbox value, but include required field)
        response = self.client.post(
            reverse("books:book_metadata_update", kwargs={"pk": self.book.pk}), {"final_title": self.final_metadata.final_title}  # Required field, no is_reviewed
        )
    ...
    def test_no_changes_message(self):
        ...
        # Submit same data
        update_data = {"final_title": self.final_metadata.final_title, "final_author": self.final_metadata.final_author}
    ...
    def test_manual_update_flag(self):
        ...
        update_data = {"final_title": "Updated Title"}
    ...
    def test_create_final_metadata_if_missing(self):
        ...
        update_data = {"final_title": "New Title"}
...
class MetadataProcessingEdgeCaseTests(TestCase):
    ...
    def setUp(self):
        ...
        self.final_metadata = FinalMetadata.objects.create(
            book=self.book, final_title="Original Title", final_author="Original Author", is_reviewed=True  # Prevent auto-sync during manual updates
        )
    ...
    def test_whitespace_only_fields(self):
        ...
        # Whitespace-only final_title should fail validation
        update_data = {"final_title": "   ", "final_author": "\t\n", "final_series": ""}  # Whitespace-only - should fail
        ...
        self.assertEqual(final_metadata.final_title, "Original Title")
        self.assertEqual(final_metadata.final_author, "Original Author")

    def test_very_long_field_values(self):
        """Test handling of very long field values"""
        long_title = "A" * 1000  # Very long title

        update_data = {"final_title": long_title}
    ...
    def test_invalid_numeric_values(self):
        ...
        update_data = {
            ...
            "isbn": "123",  # Valid field but maybe invalid ISBN format
        }

        response = self.client.post(reverse("books:book_metadata_update", kwargs={"pk": self.book.pk}), update_data)

        # Invalid publication_year should be handled gracefully
        # View might accept it or show error - check if it doesn't crash
