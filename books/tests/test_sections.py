...
class SectionsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username=username, email="test@example.com", password="testpass123")

        # Create scan folders with different content types using real temp directories
        self.ebooks_folder = ScanFolder.objects.create(name="Ebooks Folder", path=self.temp_ebooks_dir, content_type="ebooks", is_active=True)

        self.comics_folder = ScanFolder.objects.create(name="Comics Folder", path=self.temp_comics_dir, content_type="comics", is_active=True)

        self.audiobooks_folder = ScanFolder.objects.create(name="Audiobooks Folder", path=self.temp_audiobooks_dir, content_type="audiobooks", is_active=True)

        # Mixed folders are no longer supported - removed

        self.inactive_folder = ScanFolder.objects.create(name="Inactive Folder", path=self.temp_inactive_dir, content_type="ebooks", is_active=False)

        # Create data source
        self.data_source = DataSource.objects.create(name="Test Source", trust_level=0.8)

        # Create test books in different folders
        self.ebook1 = create_test_book_with_file(
            file_path="/test/ebooks/book1.epub", file_format="epub", file_size=1000000, scan_folder=self.ebooks_folder, content_type="ebook", title="Book 1"
        )

        self.ebook2 = create_test_book_with_file(
            file_path="/test/ebooks/book2.pdf", file_format="pdf", file_size=2000000, scan_folder=self.ebooks_folder, content_type="ebook", title="Book 2"
        )

        self.comic1 = create_test_book_with_file(
            file_path="/test/comics/comic1.cbr", file_format="cbr", file_size=5000000, scan_folder=self.comics_folder, content_type="comic", title="Comic 1"
        )

        self.comic2 = create_test_book_with_file(
            file_path="/test/comics/comic2.pdf", file_format="pdf", file_size=3000000, scan_folder=self.comics_folder, content_type="comic", title="Comic 2"
        )

        self.audiobook1 = create_test_book_with_file(
            file_path="/test/audiobooks/audio1.m4b", file_format="m4b", file_size=100000000, scan_folder=self.audiobooks_folder, content_type="audiobook", title="Audiobook 1"
        )

        self.audiobook2 = create_test_book_with_file(
            file_path="/test/audiobooks/audio2.mp3", file_format="mp3", file_size=50000000, scan_folder=self.audiobooks_folder, content_type="audiobook", title="Audiobook 2"
        )

        self.inactive_book = create_test_book_with_file(
            file_path="/test/inactive/book.epub", file_format="epub", file_size=1000000, scan_folder=self.inactive_folder, content_type="ebook", title="Inactive Book"
        )

        # Create authors and series with unique names
        self.author1 = Author.objects.create(first_name=f"J.K._{unique_id}", last_name="Rowling")

        self.author2 = Author.objects.create(first_name=f"Frank_{unique_id}", last_name="Miller")
        ...
        self.final_meta_ebook2 = FinalMetadata.objects.create(book=self.ebook2, final_title="Standalone Ebook", final_author="Unknown Author", is_reviewed=True)

        self.final_meta_comic1 = FinalMetadata.objects.create(
            book=self.comic1, final_title="Batman: Year One", final_author="Frank Miller", final_series="Batman", final_series_number="1", is_reviewed=True
        )

        self.final_meta_comic2 = FinalMetadata.objects.create(book=self.comic2, final_title="Standalone Comic", final_author="Alan Moore", is_reviewed=True)

        self.final_meta_audiobook2 = FinalMetadata.objects.create(book=self.audiobook2, final_title="Audiobook Folder Book", final_author="Test Author", is_reviewed=True)

        # Create BookMetadata for search testing and fallback testing
        BookMetadata.objects.create(
            book=self.ebook1, field_name="title", field_value="Harry Potter and the Philosopher's Stone", source=self.data_source, confidence=0.9, is_active=True
        )

        self.book_metadata = BookMetadata.objects.create(
            book=self.audiobook1, field_name="title", field_value="Test Audiobook", source=self.data_source, confidence=0.9, is_active=True
        )

        BookMetadata.objects.create(book=self.audiobook1, field_name="author", field_value="Test Author", source=self.data_source, confidence=0.9, is_active=True)

        # Create BookSeries relationships
        BookSeries.objects.create(book=self.ebook1, series=self.series1, series_number="1", source=self.data_source, confidence=0.9, is_active=True)
...
class HelperFunctionsTests(SectionsTestCase):
    ...
    def test_get_book_metadata_dict_no_metadata(self):
        ...
        book = create_test_book_with_file(file_path="/test/empty.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="Empty")
    ...
    def test_get_book_metadata_dict_invalid_publication_date(self):
        ...
        BookMetadata.objects.create(book=self.audiobook2, field_name="publication_date", field_value="invalid_date", source=self.data_source, confidence=0.9, is_active=True)
    ...
    def test_get_book_cover_url_local_path(self):
        ...
        with patch("django.conf.settings.MEDIA_ROOT", "/media/root/"), patch("django.conf.settings.MEDIA_URL", "/media/"):
    ...
    def test_get_book_cover_url_fallback_to_book(self):
        ...
        book = create_test_book_with_file(file_path="/test/book.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="Test Book")
    ...
    def test_get_book_cover_url_exception_handling(self):
        ...
        book_no_cover = create_test_book_with_file(file_path="/test/no_cover.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="No Cover Book")
...
class EbooksViewsTests(SectionsTestCase):
    ...
    def test_ebooks_ajax_detail_no_metadata(self):
        ...
        book = create_test_book_with_file(file_path="/test/empty.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="Empty Book")
...
class SeriesViewsTests(SectionsTestCase):
    ...
    def test_series_ajax_list_book_sorting(self):
        ...
        book3 = create_test_book_with_file(file_path="/test/book3.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="Test Book 3")

        FinalMetadata.objects.create(
            book=book3, final_title="Harry Potter and the Chamber of Secrets", final_author="J.K. Rowling", final_series="Harry Potter", final_series_number="2", is_reviewed=True
        )
...
class ComicsViewsTests(SectionsTestCase):
    ...
    def test_comics_ajax_list_series_books_sorting(self):
        ...
        comic3 = create_test_book_with_file(file_path="/test/comics/comic3.cbr", file_format="cbr", scan_folder=self.comics_folder, content_type="comic", title="Comic 3")

        FinalMetadata.objects.create(
            book=comic3, final_title="Batman: The Dark Knight Returns", final_author="Frank Miller", final_series="Batman", final_series_number="2", is_reviewed=True
        )
    ...
    def test_comics_ajax_list_standalone_sorting(self):
        ...
        comic4 = create_test_book_with_file(file_path="/test/comics/comic4.cbz", file_format="cbz", scan_folder=self.comics_folder, content_type="comic", title="Comic 4")

        FinalMetadata.objects.create(book=comic4, final_title="Another Standalone", final_author="Different Author", is_reviewed=True)
...
class AudiobooksViewsTests(SectionsTestCase):
    ...
    def test_audiobooks_ajax_list_data_structure(self):
        ...
        required_fields = ["id", "title", "author", "narrator", "duration", "file_format", "file_size", "last_scanned", "series_name", "series_position"]
...
class EdgeCasesAndErrorHandlingTests(SectionsTestCase):
    ...
    def test_books_without_scan_folder(self):
        ...
        book = create_test_book_with_file(file_path="/test/orphan.epub", file_format="epub", scan_folder=None, content_type="ebook", title="Orphan Book")
    ...
    def test_books_with_invalid_series_position(self):
        ...
        comic = create_test_book_with_file(file_path="/test/invalid.cbr", file_format="cbr", scan_folder=self.comics_folder, content_type="comic", title="Invalid Comic")

        FinalMetadata.objects.create(
            book=comic, final_title="Invalid Position Comic", final_author="Test Author", final_series="Test Series", final_series_number="invalid", is_reviewed=True
        )
    ...
    def test_missing_final_metadata_relationships(self):
        ...
        book = create_test_book_with_file(file_path="/test/no_meta.epub", file_format="epub", scan_folder=self.ebooks_folder, content_type="ebook", title="No Meta Book")
...
class PerformanceTests(SectionsTestCase):
    ...
    def test_pagination_limit(self):
        ...
        for i in range(600):  # More than the 500 limit
            book = create_test_book_with_file(file_path=f"/test/book{i}.epub", file_format="epub", content_type="ebook", title=f"Book {i}", scan_folder=self.ebooks_folder)
            books.append(book)
