...
class SeriesViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book1 = create_test_book_with_file(file_path="/test/book1.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder)

        self.book2 = create_test_book_with_file(file_path="/test/book2.pdf", file_size=2000000, file_format="pdf", scan_folder=self.scan_folder)

        self.book3 = create_test_book_with_file(file_path="/test/book3.mobi", file_size=1500000, file_format="mobi", scan_folder=self.scan_folder)

        self.book4 = create_test_book_with_file(file_path="/test/book4.epub", file_size=800000, file_format="epub", scan_folder=self.scan_folder)
        ...
        self.final_meta2 = FinalMetadata.objects.create(
            book=self.book2, final_title="The Fellowship of the Ring", final_author="J.R.R. Tolkien", final_series="Lord of the Rings", final_series_number="1", is_reviewed=True
        )

        self.final_meta3 = FinalMetadata.objects.create(
            book=self.book3, final_title="The Colour of Magic", final_author="Terry Pratchett", final_series="Discworld", final_series_number="1", is_reviewed=True
        )

        # Book without series
        self.final_meta4 = FinalMetadata.objects.create(book=self.book4, final_title="Standalone Book", final_author="Some Author", final_series="", is_reviewed=True)  # No series

        # Create some BookSeries relationships (formal relationships)
        self.book_series1 = BookSeries.objects.create(book=self.book1, series=self.series1, series_number=1, source=self.data_source, confidence=0.9, is_active=True)

        self.book_series2 = BookSeries.objects.create(book=self.book2, series=self.series2, series_number=1, source=self.data_source, confidence=0.9, is_active=True)
...
class GetBookMetadataDictTests(SeriesViewsTestCase):
    ...
    def test_get_metadata_without_final_metadata(self):
        ...
        book_no_meta = create_test_book_with_file(file_path="/test/no_meta.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder)
...
class SeriesMainViewTests(SeriesViewsTestCase):
    ...
    def test_series_count_excludes_empty_series(self):
        ...
        book_empty_series = create_test_book_with_file(file_path="/test/empty_series.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder)

        FinalMetadata.objects.create(book=book_empty_series, final_title="Book Without Series", final_author="Author", final_series="", is_reviewed=True)  # Empty series
    ...
    def test_series_count_excludes_null_series(self):
        ...
        book_null_series = create_test_book_with_file(file_path="/test/null_series.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder)

        FinalMetadata.objects.create(book=book_null_series, final_title="Book Without Series", final_author="Author", final_series=None, is_reviewed=True)  # Null series
...
class SeriesAjaxListTests(SeriesViewsTestCase):
    ...
    def test_series_ajax_list_book_sorting(self):
        ...
        book_hp2 = create_test_book_with_file(file_path="/test/hp2.epub", file_size=1100000, file_format="epub", scan_folder=self.scan_folder)
        ...
        book_hp_no_pos = create_test_book_with_file(file_path="/test/hp_no_pos.epub", file_size=1050000, file_format="epub", scan_folder=self.scan_folder)
    ...
    def test_series_ajax_list_excludes_placeholder_books(self):
        ...
        placeholder_book = create_test_book_with_file(
            file_path="/test/placeholder.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder, is_placeholder=True  # This should be excluded
        )

        FinalMetadata.objects.create(
            book=placeholder_book, final_title="Placeholder Book", final_author="Author", final_series="Test Series", final_series_number="1", is_reviewed=True
        )
    ...
    def test_series_ajax_list_aggregates_data_correctly(self):
        ...
        book_discworld2 = create_test_book_with_file(file_path="/test/discworld2.epub", file_size=1200000, file_format="mobi", scan_folder=self.scan_folder)  # Different format

        FinalMetadata.objects.create(
            book=book_discworld2, final_title="The Light Fantastic", final_author="Terry Pratchett", final_series="Discworld", final_series_number="2", is_reviewed=True
        )
