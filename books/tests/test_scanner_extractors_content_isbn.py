...
class ContentISBNExtractorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.epub_book = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "test.epub"), file_format="epub", file_size=1024000, scan_folder=self.scan_folder)

        self.pdf_book = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "test.pdf"), file_format="pdf", file_size=1024000, scan_folder=self.scan_folder)

        # Ensure CONTENT_SCAN data source exists
        self.content_source, created = DataSource.objects.get_or_create(name=DataSource.CONTENT_SCAN, defaults={"trust_level": 0.85})
    ...
    def test_extract_isbn_from_content_unsupported_format(self):
        ...
        unsupported_book = create_test_book_with_file(file_path="/test/scan/folder/test.txt", file_format="txt", file_size=1024, scan_folder=self.scan_folder)
    ...
    def test_extract_isbn_from_content_mobi_not_implemented(self):
        ...
        mobi_book = create_test_book_with_file(file_path="/test/scan/folder/test.mobi", file_format="mobi", file_size=1024000, scan_folder=self.scan_folder)
    ...
    def test_save_content_isbns_success(self, mock_extract):
        ...
        metadata = BookMetadata.objects.filter(book=self.epub_book, field_name="isbn", source=self.content_source)
    ...
    def test_save_content_isbns_duplicate_handling(self, mock_extract):
        ...
        BookMetadata.objects.create(book=self.epub_book, field_name="isbn", field_value="9780134685991", source=self.content_source, confidence=0.85)
    ...
    def test_bulk_scan_content_isbns(self, mock_save):
        ...
        book2 = create_test_book_with_file(file_path="/test/scan/folder/test2.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)
        ...
        def mock_save_side_effect(book):
            if book == self.epub_book:
                ...
                BookMetadata.objects.create(book=book, field_name="isbn", field_value="9780134685991", source=self.content_source, confidence=0.85)
