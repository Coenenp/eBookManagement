...
class UserRegisterFormTests(TestCase):
    ...
    def test_user_register_form_password_mismatch(self):
        ...
        form_data = {"username": "testuser", "email": "test@example.com", "password1": "testpassword123", "password2": "differentpassword"}
        form = UserRegisterForm(data=form_data)
        self.assertFalse(form.is_valid())

    def test_user_register_form_invalid_email(self):
        """Test UserRegisterForm with invalid email"""
        form_data = {"username": "testuser", "email": "invalid-email", "password1": "testpassword123", "password2": "testpassword123"}
        form = UserRegisterForm(data=form_data)
        self.assertFalse(form.is_valid())
...
class ScanFolderFormTests(TestCase):
    ...
    def test_scan_folder_form_valid(self):
        ...
        form_data = {"name": "Test Folder", "path": self.temp_dir, "content_type": "ebooks", "language": "en", "is_active": True}
        form = ScanFolderForm(data=form_data)
        self.assertTrue(form.is_valid())

    def test_scan_folder_form_empty_path(self):
        """Test ScanFolderForm with empty path"""
        form_data = {"name": "Test Folder", "path": "", "content_type": "ebooks", "language": "en", "is_active": True}
        form = ScanFolderForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_scan_folder_form_accepts_any_path_string(self):
        ...
        form_data = {"name": "Test Folder", "path": self.temp_dir, "content_type": "ebooks", "language": "en", "is_active": True}  # Use actual existing path
        form = ScanFolderForm(data=form_data)
        self.assertTrue(form.is_valid())
    ...
    def test_scan_folder_form_accepts_file_paths(self):
        ...
        form_data = {"name": "Test Folder", "path": test_file, "content_type": "ebooks", "language": "en", "is_active": True}
        form = ScanFolderForm(data=form_data)
    ...
    def test_scan_folder_form_path_cleaning(self):
        ...
        form_data = {"name": "Test Folder", "path": f"  {self.temp_dir}  ", "content_type": "ebooks", "language": "en", "is_active": True}  # Add whitespace around temp_dir
        form = ScanFolderForm(data=form_data)
        self.assertTrue(form.is_valid())
...
class BookSearchFormTests(TestCase):
    ...
    def test_book_search_form_empty_optional_fields(self):
        ...
        form_data = {"search_query": "", "language": "", "file_format": "", "has_placeholder": "", "is_reviewed": "", "confidence_level": "", "has_cover": ""}
        form = BookSearchForm(data=form_data)
        self.assertTrue(form.is_valid())
...
class MetadataReviewFormTests(TestCase):
    ...
    def test_metadata_review_form_cover_upload(self):
        ...
        uploaded_file = SimpleUploadedFile("test_cover.jpg", img_io.read(), content_type="image/jpeg")

        form_data = {
            "final_title": "Valid Title",
            "final_author": "Valid Author",
        }
        file_data = {"new_cover_upload": uploaded_file}
        form = MetadataReviewForm(data=form_data, files=file_data, instance=self.final_metadata, book=self.book)
        self.assertTrue(form.is_valid())
...
class BookEditFormTests(TestCase):
    ...
    def test_book_edit_form_valid(self):
        ...
        form_data = {"file_format": "pdf", "cover_path": "/new/cover/path.jpg", "opf_path": "/new/opf/path.opf", "is_placeholder": True, "is_duplicate": False}
...
class BookCoverFormTests(TestCase):
    ...
    def test_book_cover_form_valid(self):
        ...
        form_data = {"cover_path": "/path/to/cover.jpg", "confidence": 0.8, "width": 800, "height": 1200, "format": "jpg"}
    ...
    def test_book_cover_form_confidence_validation(self):
        ...
        form_data = {"cover_path": "/path/to/cover.jpg", "confidence": 1.5, "width": 800, "height": 1200, "format": "jpg"}  # Invalid confidence
        form = BookCoverForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_book_cover_form_negative_confidence(self):
        ...
        form_data = {"cover_path": "/path/to/cover.jpg", "confidence": -0.1, "width": 800, "height": 1200, "format": "jpg"}  # Invalid confidence
        form = BookCoverForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn("confidence", form.errors)
    ...
    def test_book_cover_form_invalid_confidence_string(self):
        ...
        form_data = {"cover_path": "/path/to/cover.jpg", "confidence": "invalid", "width": 800, "height": 1200, "format": "jpg"}
        form = BookCoverForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn("confidence", form.errors)
...
class BulkUpdateFormTests(TestCase):
    ...
    def test_bulk_update_form_valid(self):
        ...
        form_data = {"action": "mark_reviewed", "selected_books": "1,2,3"}
    ...
    def test_bulk_update_form_no_action(self):
        ...
        form_data = {"action": "", "selected_books": "1,2,3"}
        form = BulkUpdateForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_bulk_update_form_no_books_selected(self):
        ...
        form_data = {"action": "mark_reviewed", "selected_books": ""}
        form = BulkUpdateForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_bulk_update_form_invalid_book_ids(self):
        ...
        form_data = {"action": "mark_reviewed", "selected_books": "1,invalid,3"}
        form = BulkUpdateForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn("selected_books", form.errors)
...
class AdvancedSearchFormTests(TestCase):
    ...
    def test_advanced_search_form_invalid_year_range(self):
        ...
        form_data = {"publication_year_from": 2023, "publication_year_to": 2000}  # From year > To year
        form = AdvancedSearchForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_advanced_search_form_invalid_confidence_range(self):
        ...
        form_data = {"confidence_min": 0.8, "confidence_max": 0.3}  # Min > Max
        form = AdvancedSearchForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn("__all__", form.errors)
    ...
    def test_advanced_search_form_invalid_from_year(self):
        ...
        form_data = {"publication_year_from": 999}  # Too old
        form = AdvancedSearchForm(data=form_data)
        self.assertFalse(form.is_valid())
    ...
    def test_advanced_search_form_invalid_to_year(self):
        ...
        form_data = {"publication_year_to": 3000}  # Too future
        form = AdvancedSearchForm(data=form_data)
        self.assertFalse(form.is_valid())
...
class UserProfileFormTests(TestCase):
    ...
    def test_user_profile_form_valid(self):
        ...
        form_data = {"theme": "darkly", "items_per_page": 25, "show_covers_in_list": False, "default_view_mode": "grid", "share_reading_progress": True}
    ...
    def test_user_profile_form_help_text(self):
        ...
        self.assertEqual(form.fields["theme"].help_text, "Choose your preferred visual theme")
        self.assertEqual(form.fields["items_per_page"].help_text, "Number of books to display per page (10-200)")
...
class FormIntegrationTests(TestCase):
    ...
    def test_form_validation_chain(self):
        ...
        form_data = {"final_title": "   ", "final_author": "Valid Author", "isbn": "123", "publication_year": 999}  # Whitespace only - should fail  # Invalid ISBN  # Invalid year
    ...
    def test_bulk_form_edge_cases(self):
        ...
        form_data = {"action": "mark_reviewed", "selected_books": "5"}
        form = BulkUpdateForm(data=form_data)
        self.assertTrue(form.is_valid())
    ...
    def test_form_error_messages(self):
        ...
        form_data = {"name": "", "path": "/test/path", "content_type": "ebooks", "language": "en", "is_active": True}  # Required field left empty
