        ...
        for rating, expected_score in test_cases:
            with self.subTest(rating=rating):
                ...
                feedback = AIFeedback.objects.create(
                    book=book,
                    user=self.user,
                    original_filename=f'test_book_{rating}.epub',
                    ai_predictions='{}',
                    user_corrections='{}',
...
class AIViewsTests(TestCase):
    ...
    def test_ajax_submit_ai_feedback(self):
        ...
        request = self.factory.post(
            f'/ajax/book/{self.book.id}/ai-feedback/',
    ...
    def test_ajax_ai_model_status(self, mock_recognizer):
        ...
        mock_instance = Mock()
        mock_instance.models_exist.return_value = True
        mock_instance.get_training_data_stats.return_value = {
            'total_samples': 50,
            'accuracy': 0.85
        }
        mock_recognizer.return_value = mock_instance
    ...
    def test_ajax_retrain_ai_models(self, mock_call_command):
        ...
        for i in range(6):
            ...
            AIFeedback.objects.create(
                book=book,
                user=self.user,
                original_filename=f'test_book_{i}.epub',
                ai_predictions='{}',
                user_corrections='{}',
                feedback_rating=4,
...
class AIIntegrationTests(TestCase):
    ...
    def test_scanner_ai_integration(self, mock_init_ai):
        ...
        mock_recognizer.predict_metadata.return_value = {
            'title': ('Test Title', 0.9),
            'author': ('Test Author', 0.8),
            'series': ('Test Series', 0.7),
            'volume': ('1', 0.6)
        }
    ...
    def test_ai_fallback_to_traditional(self, mock_init_ai):
        ...
        mock_recognizer = Mock()
        mock_recognizer.predict_metadata.return_value = {
            'title': ('Uncertain Title', 0.3),
            'author': ('Uncertain Author', 0.2),
            'series': ('', 0.1),
            'volume': ('', 0.1)
        }
...
class AIPerformanceTests(TestCase):
    ...
    def test_large_dataset_handling(self):
        ...
        for i in range(1000):
            large_dataset.append({
                'filename': f'Author{i} - Book{i}.epub',
                'title': f'Book{i}',
                'author': f'Author{i}',
                'series': '',
                'volume': ''
    ...
    def test_unicode_filename_handling(self):
        ...
        unicode_filenames = [
            ...
            "山田太郎 - 日本の本.epub",
            "Владимир Петров - Русская Книга.epub"
        ]
