...
class RenamingViewsAdditionalTests(TestCase):
    ...
    def setUp(self):
        ...
        User = get_user_model()
        self.user = User.objects.create_user(username="testuser_additional", password="testpass123")
        ...
        self.book_with_special_chars = create_test_book_with_file(
            file_path="/test/books/Special & Characters! @#$.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder
        )
        ...
        self.book_long_title = create_test_book_with_file(file_path="/test/books/very_long_filename.epub", file_format="epub", file_size=2000000, scan_folder=self.scan_folder)
    ...
    def test_renaming_with_special_characters(self):
        ...
        response = self.client.post(preview_url, {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [self.book_with_special_chars.id]})
    ...
    def test_renaming_with_very_long_titles(self):
        ...
        preview_url = reverse("books:renamer_preview_pattern")
        response = self.client.post(preview_url, {"folder_pattern": "${author_last}", "filename_pattern": "${title}.${ext}", "book_ids": [self.book_long_title.id]})
    ...
    def test_pattern_validation_edge_cases(self):
        ...
        # Test empty pattern
        response = self.client.post(validate_url, {"pattern": "", "type": "filename"})
        data = json.loads(response.content)
        ...
        for char in invalid_chars:
            response = self.client.post(validate_url, {"pattern": f"title{char}author", "type": "filename"})
            data = json.loads(response.content)
        ...
        def rename_operation(thread_id):
            try:
                ...
                response = self.client.post(
                    preview_url, {"folder_pattern": f"Thread_{thread_id}", "filename_pattern": "${title}.${ext}", "book_ids": [self.book_with_special_chars.id]}
                )
    ...
    def test_memory_usage_with_large_batch(self):
        ...
        for i in range(20):  # Reduced for CI
            book = create_test_book_with_file(file_path=f"/test/batch/book_{i}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

            FinalMetadata.objects.create(book=book, final_title=f"Batch Test Book {i}", final_author=f"Author {i}", is_reviewed=True)
        ...
        response = self.client.post(preview_url, {"folder_pattern": "${author_last}", "filename_pattern": "${title}.${ext}", "book_ids": book_ids})
    ...
    def test_file_permission_handling(self, mock_access, mock_exists):
        ...
        response = self.client.post(
            execute_url, {"folder_pattern": "${author_last}", "filename_pattern": "${title}.${ext}", "book_ids": [self.book_with_special_chars.id], "dry_run": False}
        )
    ...
    def test_unicode_filename_handling(self):
        ...
        unicode_book = create_test_book_with_file(file_path="/test/unicode/café.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        FinalMetadata.objects.create(book=unicode_book, final_title="Café François 中文 العربية", final_author="José María Azñar", is_reviewed=True)

        preview_url = reverse("books:renamer_preview_pattern")
        response = self.client.post(preview_url, {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [unicode_book.id]})
    ...
    def test_network_path_handling(self):
        ...
        network_book = create_test_book_with_file(file_path="//server/share/books/network_book.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        FinalMetadata.objects.create(book=network_book, final_title="Network Book", final_author="Network Author", is_reviewed=True)

        preview_url = reverse("books:renamer_preview_pattern")
        response = self.client.post(preview_url, {"folder_pattern": "${author_last}", "filename_pattern": "${title}.${ext}", "book_ids": [network_book.id]})
    ...
    def test_pattern_complexity_limits(self):
        ...
        response = self.client.post(validate_url, {"pattern": complex_pattern, "type": "filename"})
    ...
    def test_renaming_robustness_under_load(self):
        ...
        for i in range(10):
            preview_url = reverse("books:renamer_preview_pattern")
            response = self.client.post(
                preview_url, {"folder_pattern": f"Load_Test_{i}", "filename_pattern": "${title}_${counter}.${ext}", "book_ids": [self.book_with_special_chars.id]}
            )
    ...
    def test_cross_platform_path_compatibility(self):
        ...
        for pattern, description in test_patterns:
            response = self.client.post(preview_url, {"folder_pattern": pattern, "filename_pattern": "${title}.${ext}", "book_ids": [self.book_with_special_chars.id]})
    ...
    def test_metadata_edge_cases_handling(self):
        ...
        edge_case_book = create_test_book_with_file(file_path="/test/edge/edge_case.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
        ...
        preview_url = reverse("books:renamer_preview_pattern")
        response = self.client.post(preview_url, {"folder_pattern": "${author_sort}/${series_name}", "filename_pattern": "${title}.${ext}", "book_ids": [edge_case_book.id]})
    ...
    def test_renaming_performance_benchmarks(self):
        ...
        for i in range(50):  # Moderate size for CI
            book = create_test_book_with_file(file_path=f"/test/perf/performance_book_{i:03d}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
        ...
        response = self.client.post(validate_url, {"pattern": "${author_sort}/${series_name}/${series_number:02d} - ${title}.${ext}", "type": "filename"})
        ...
        response = self.client.post(
            preview_url, {"folder_pattern": "${author_sort}/${series_name}", "filename_pattern": "${series_number:02d} - ${title}.${ext}", "book_ids": book_ids}
        )
