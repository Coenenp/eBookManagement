...
class RescanEndpointTestCase(TestCase):
    ...
    def test_rescan_existing_book_post_request(self):
        ...
        url = reverse("books:rescan_external_metadata", kwargs={"book_id": self.book.id})
        response = self.client.post(url, data={"sources": ["google"], "title_search": "Test Book", "author_search": "Test Author"})
        ...
        if "application/json" in response.get("Content-Type", ""):
            try:
                ...
                self.assertTrue(response_data.get("success", False) or "job_id" in response_data or "status" in response_data)
            except ValueError:
    ...
    def test_rescan_with_json_data(self):
        ...
        response = self.client.post(
            url, data=json.dumps({"sources": ["google", "openlibrary"], "title_search": "Test JSON Book", "author_search": "JSON Author"}), content_type="application/json"
        )
