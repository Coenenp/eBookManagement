...
class ComicRenamerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.content_source, _ = DataSource.objects.get_or_create(name=DataSource.CONTENT_SCAN, defaults={"trust_level": 0.85})
    ...
    def create_comic_book(self, filename, series_name, issue_type="main_series", issue_number=None, publisher=None):
        ...
        scan_folder, _ = ScanFolder.objects.get_or_create(path=self.test_dir, defaults={"name": "Test Comics", "is_active": True, "content_type": "comics"})

        book = Book.objects.create(scan_folder=scan_folder, content_type="comic")

        BookFile.objects.create(book=book, file_path=f"{self.test_dir}/{filename}", file_size=1000000, file_format="cbz")
        ...
        for field_name, field_value in metadata_entries:
            BookMetadata.objects.create(book=book, field_name=field_name, field_value=field_value, source=self.content_source, confidence=0.9)
    ...
    def test_generate_comic_file_path_main_series(self):
        ...
        book = self.create_comic_book("spider-man-001.cbz", "Spider-Man", "main_series", 1, "Marvel Comics")
    ...
    def test_generate_comic_file_path_annual(self):
        ...
        book = self.create_comic_book("spider-man-annual-1.cbz", "Spider-Man", "annual", publisher="Marvel Comics")

        # Add annual number metadata
        BookMetadata.objects.create(book=book, field_name="annual_number", field_value="1", source=self.content_source, confidence=0.9)
    ...
    def test_generate_comic_file_path_special(self):
        ...
        book = self.create_comic_book("spider-man-special.cbz", "Spider-Man", "special", publisher="Marvel Comics")
    ...
    def test_generate_comic_file_path_collection(self):
        ...
        book = self.create_comic_book("spider-man-tpb-vol-1.cbz", "Spider-Man", "collection", publisher="Marvel Comics")
    ...
    def test_generate_comic_filename(self):
        ...
        test_cases = [
            {"issue_type": "main_series", "metadata": {"issue_number": 1}, "series": "Spider-Man", "expected": "Spider-Man #001 - Amazing Spider-Man #1"},
            {"issue_type": "annual", "metadata": {"annual_number": 1}, "series": "Spider-Man", "expected": "Spider-Man Annual #1 - Amazing Spider-Man #1"},
            {"issue_type": "special", "metadata": {}, "series": "Spider-Man", "expected": "Spider-Man Special - Amazing Spider-Man #1"},
            {"issue_type": "one_shot", "metadata": {}, "series": "Spider-Man", "expected": "Spider-Man One-Shot - Amazing Spider-Man #1"},
        ]

        for case in test_cases:
            with self.subTest(issue_type=case["issue_type"]):
                result = view._generate_comic_filename(case["issue_type"], case["metadata"], case["series"], book)
    ...
    def test_comic_series_analysis_integration(self):
        ...
        for i in range(1, 4):  # Issues 1, 2, 3
            book = self.create_comic_book(f"spider-man-{i:03d}.cbz", "Spider-Man", "main_series", i, "Marvel Comics")
            books.append(book)

        # Add an annual
        annual = self.create_comic_book("spider-man-annual-1.cbz", "Spider-Man", "annual", publisher="Marvel Comics")

        BookMetadata.objects.create(book=annual, field_name="annual_number", field_value="1", source=self.content_source, confidence=0.9)
    ...
    def test_comic_filter_by_issue_type(self, mock_analysis):
        ...
        # Setup mock analysis
        mock_analysis.return_value = {"all_series": [], "complete_series": [], "incomplete_series": [], "total_series": 0, "complete_count": 0, "incomplete_count": 0}

        # Create comics of different types
        main_issue = self.create_comic_book("spider-man-001.cbz", "Spider-Man", "main_series", 1)

        annual = self.create_comic_book("spider-man-annual-1.cbz", "Spider-Man", "annual")

        special = self.create_comic_book("spider-man-special.cbz", "Spider-Man", "special")
        ...
        view.request.GET.get.side_effect = lambda key, default="": {"issue_type": "main_series"}.get(key, default)
