...
class WizardCompleteIntegrationTests(TestCase):
    ...
    def test_complete_wizard_flow_with_prepopulation(self):
        ...
        response = self.client.post(reverse('books:wizard_scrapers'), {
            'google_books': 'test_api_key_12345',
    ...
    def test_wizard_prepopulation_on_second_run(self):
        ...
        self.assertTrue(
            'existing_key' in content or
            'value=' in content or  # Pre-filled input values
            'is-valid' in content   # Bootstrap validation classes
        )
    ...
    def test_wizard_validation_and_error_handling(self):
        ...
        response = self.client.post(reverse('books:wizard_folders'), {
            'action': 'add_folder',
            'folder_path': '/absolutely/nonexistent/path/12345',
    ...
    def test_wizard_css_and_javascript_functionality(self):
        ...
        self.assertTrue(
            'wizard-complete-page' in content or
            'feature-highlights-row' in content or
            'wizard' in content
        )

        # Should have JavaScript for button handling
        self.assertTrue(
            'fa-spin' in content or
    ...
    def test_wizard_responsive_design_elements(self):
        ...
        for url_name in wizard_urls:
            ...
            self.assertTrue(
                'col-' in content or  # Bootstrap grid
                'responsive' in content or
                'mobile' in content or
                'viewport' in content
            )
    ...
    def test_wizard_accessibility_features(self):
        ...
        self.assertTrue(
            'aria-' in content or  # ARIA attributes
            'alt=' in content or   # Alt text for images
            'label' in content or  # Form labels
            'title=' in content    # Tooltips/titles
        )
    ...
    def test_wizard_configuration_summary(self):
        ...
        SetupWizard.objects.create(
            user=self.user,
            scraper_config={'google_books': 'test_key'},
            folder_content_types={'Books': 'ebooks'}
        )

        ScanFolder.objects.create(
            path=self.temp_dir,
            name='Summary Test Folder'
        )
        ...
        self.assertTrue(
            'Summary Test Folder' in content or
            'configuration' in content or
            'setup' in content
        )
        ...
        if hasattr(self.client, 'post'):
            with patch('os.path.exists', return_value=True):
                response = self.client.post(
                    reverse('books:wizard_validate_folder') if
                    'wizard_validate_folder' in [url.name for url in reverse.__self__.urlpatterns]
                    else '/ajax/validate-folder/',
                    {'folder_path': self.temp_dir},
    ...
    def test_wizard_mobile_compatibility(self):
        ...
        self.assertTrue(
            'mobile' in content.lower() or
            'responsive' in content.lower() or
            'viewport' in content or
            'col-sm' in content  # Bootstrap mobile classes
        )
