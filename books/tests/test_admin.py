...
class AdminSetupTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.client = Client()
        self.superuser = User.objects.create_superuser(username="admin", email="admin@test.com", password="testpass123")
...
class AdminModelAccessTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create test data
        self.scan_folder = ScanFolder.objects.create(name="Test Folder", path=self.temp_dir, language="en")

        self.book = create_test_book_with_file(
            file_path="/test/book.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder, content_type="ebook", title="Test Book"
        )
...
class AdminModelDetailTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(
            file_path="/test/book.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder, content_type="ebook", title="Test Book Admin Detail"
        )
...
class AdminInlineTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(
            file_path="/test/book.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder, content_type="ebook", title="Test Book Inline"
        )
        ...
        self.book_title = BookTitle.objects.create(book=self.book, title="Test Book", source=self.data_source, confidence=0.9)

        self.book_author = BookAuthor.objects.create(book=self.book, author=self.author, source=self.data_source, confidence=0.8)
...
class AdminFilteringTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book1 = create_test_book_with_file(
            file_path="/test/book1.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder, content_type="ebook", title="Book 1"
        )

        self.book2 = create_test_book_with_file(file_path="/test/book2.pdf", file_format="pdf", file_size=2048, scan_folder=self.scan_folder, content_type="ebook", title="Book 2")
...
class AdminFunctionalityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(
            file_path="/test/book.epub", file_format="epub", file_size=1048576, scan_folder=self.scan_folder, content_type="ebook", title="Test Book"  # 1MB
        )
    ...
    def test_datasource_admin_display(self):
        ...
        _ = DataSource.objects.create(name="Test Source", trust_level=0.85)
...
class AdminPermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create regular user
        self.regular_user = User.objects.create_user(username="user", email="user@test.com", password="testpass123")

        # Create staff user (but not superuser)
        self.staff_user = User.objects.create_user(username="staff", email="staff@test.com", password="testpass123", is_staff=True)
...
class AdminCustomMethodTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.author = Author.objects.create(name="John Doe", first_name="John", last_name="Doe")

        self.data_source = DataSource.objects.create(name="Test Source", trust_level=0.8)

        self.book_author = BookAuthor.objects.create(book=self.book, author=self.author, source=self.data_source, confidence=0.9)
    ...
    def test_book_admin_file_size_mb_unknown(self):
        ...
        book_no_size = create_test_book_with_file(file_path="/test/unknown.epub", file_format="epub", scan_folder=self.scan_folder, content_type="ebook", title="Unknown Size Book")
    ...
    def setUp(self):
        ...
        for i in range(3):
            book = create_test_book_with_file(
                file_path=f"/test/book{i}.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder, content_type="ebook", title=f"Test Book {i}"
            )
...
class AdminIntegrationTestCase(TestCase):
    ...
    def test_full_admin_workflow(self):
        ...
        response = self.client.post(
            "/admin/books/scanfolder/add/", {"name": "Integration Test Folder", "path": self.temp_dir, "content_type": "ebooks", "language": "en", "is_active": True}
        )
    ...
    def test_admin_model_relationships(self):
        ...
        scan_folder = ScanFolder.objects.create(name="Relationship Test", path=temp_rel_dir, language="en")

        book = create_test_book_with_file(
            file_path="/test/relationships/book.epub", file_format="epub", file_size=1024, scan_folder=scan_folder, content_type="ebook", title="Relationship Test Book"
        )
