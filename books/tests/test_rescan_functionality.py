...
class RescanFunctionalityTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.scan_folder = ScanFolder.objects.create(name="Test Scan Folder", path=self.temp_dir, content_type="ebooks")

        # Create a test book
        self.book = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "book.epub"), file_format="epub", file_size=1024, scan_folder=self.scan_folder)

        # Create a data source
        self.source, created = DataSource.objects.get_or_create(name=DataSource.MANUAL, defaults={"trust_level": 0.8})

        # Create some initial metadata
        BookTitle.objects.create(book=self.book, title="Test Book Title", source=self.source, confidence=0.8)

        author = Author.objects.create(name="Test Author")
        BookAuthor.objects.create(book=self.book, author=author, source=self.source, confidence=0.8)

        # Create final metadata
        FinalMetadata.objects.create(book=self.book, final_title="Test Book Title", final_author="Test Author", isbn="9780062498557", is_reviewed=True)
    ...
    def test_rescan_endpoint_exists(self):
        ...
        data = {"sources": ["google", "openlibrary"], "title_search": "Test Book Title", "author_search": "Test Author"}
    ...
    def test_rescan_with_form_data(self):
        ...
        data = {"sources[]": ["google"], "clear_existing": "false", "force_refresh": "false", "title_override": "Custom Title", "author_override": "Custom Author"}
    ...
    def test_rescan_invalid_book(self):
        ...
        data = {"sources": ["google"], "title_search": "Test", "author_search": "Test"}
    ...
    def test_metadata_counts(self):
        ...
        data = {"sources": ["google"], "title_search": "Test Title", "author_search": "Test Author"}
...
class ExternalQueryFunctionTestCase(TestCase):
    ...
    def test_query_metadata_and_covers_with_terms_function_exists(self):
        ...
        result = query_metadata_and_covers_with_terms(book=self.book, search_title="Test Book", search_author="Test Author", search_isbn="")
