...
class ComicVineAPITests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.comicvine_source, _ = DataSource.objects.get_or_create(name=DataSource.COMICVINE, defaults={"trust_level": 0.8})
    ...
    def test_make_request_success(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response_data = {"status_code": 1, "results": [{"id": 123, "name": "Test Comic"}]}
    ...
    def test_make_request_api_error(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response_data = {"status_code": 100, "error": "Invalid API key"}  # Error status
        mock_client.make_request.return_value = mock_response_data
        mock_get_client.return_value = mock_client
    ...
    def test_search_volumes(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": [{"id": 123, "name": "Amazing Spider-Man", "start_year": 1963}, {"id": 456, "name": "Spider-Man", "start_year": 1990}]}
    ...
    def test_search_issues(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": [{"id": 789, "name": "Origin", "issue_number": "1"}, {"id": 790, "name": "First Hunt", "issue_number": "2"}]}
        mock_client.make_request.return_value = mock_response
        mock_get_client.return_value = mock_client
    ...
    def test_get_volume_details(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": {"id": 123, "name": "Amazing Spider-Man", "count_of_issues": 800}}
        mock_client.make_request.return_value = mock_response
        mock_get_client.return_value = mock_client
    ...
    def test_get_issue_details(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": {"id": 789, "name": "Origin", "issue_number": "1"}}
        mock_client.make_request.return_value = mock_response
        mock_get_client.return_value = mock_client
    ...
    def test_search_issue(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": [{"id": 789, "name": "Amazing Spider-Man #1", "issue_number": "1"}]}
        mock_client.make_request.return_value = mock_response
        mock_get_client.return_value = mock_client
    ...
    def test_search_issue_no_results(self, mock_get_client):
        ...
        mock_client = Mock()
        mock_response = {"status_code": 1, "results": []}
        mock_client.make_request.return_value = mock_response
        mock_get_client.return_value = mock_client
...
class ComicVineMetadataExtractionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(file_path="/test/spider-man.cbz", file_format="cbz", file_size=50000000, scan_folder=self.scan_folder)
    ...
    def test_save_volume_metadata_basic(self):
        ...
        description_metadata = BookMetadata.objects.filter(book=self.book, field_name="description", source=self.comicvine_source).first()
        self.assertIsNotNone(description_metadata)
        self.assertIn("Spider-Man", description_metadata.field_value)

        # Check publication year
        year_metadata = BookMetadata.objects.filter(book=self.book, field_name="publication_year", source=self.comicvine_source).first()
    ...
    def test_save_issue_metadata_basic(self):
        ...
        issue_data = {
            ...
            "person_credits": [{"name": "Stan Lee", "role": "writer"}, {"name": "Steve Ditko", "role": "artist"}],
        }

        _save_issue_metadata(self.book, issue_data, self.comicvine_source)

        # Check title was saved
        book_title = BookTitle.objects.filter(book=self.book, source=self.comicvine_source).first()
        self.assertIsNotNone(book_title)
        self.assertEqual(book_title.title, "Origin")

        # Check description
        description = BookMetadata.objects.filter(book=self.book, field_name="description", source=self.comicvine_source).first()
        self.assertIsNotNone(description)

        # Check publication year extracted from cover date
        year = BookMetadata.objects.filter(book=self.book, field_name="publication_year", source=self.comicvine_source).first()
        ...
        book_author = BookAuthor.objects.filter(book=self.book, author=stan_lee, source=self.comicvine_source).first()
    ...
    def test_save_creators_main_author_detection(self):
        ...
        person_credits = [{"name": "Stan Lee", "role": "writer"}, {"name": "Steve Ditko", "role": "artist"}, {"name": "John Doe", "role": "colorist"}]

        _save_creators(self.book, person_credits, self.comicvine_source)

        # Check that writer is marked as main author
        stan_lee_author = BookAuthor.objects.filter(book=self.book, author__name="Stan Lee", source=self.comicvine_source).first()
        ...
        steve_ditko_author = BookAuthor.objects.filter(book=self.book, author__name="Steve Ditko", source=self.comicvine_source).first()
    ...
    def test_save_creators_limit(self):
        ...
        person_credits = [{"name": f"Creator {i}", "role": "contributor"} for i in range(10)]

        _save_creators(self.book, person_credits, self.comicvine_source)

        # Should only save 5 creators
        author_count = BookAuthor.objects.filter(book=self.book, source=self.comicvine_source).count()
    ...
    def test_save_cover_from_url_success(self, mock_image_open, mock_requests_get):
        ...
        with patch("os.path.getsize", return_value=12345):
            _save_cover_from_url(self.book, "https://comicvine.gamespot.com/image.jpg", self.comicvine_source)

        # Check that cover was saved to database
        book_cover = BookCover.objects.filter(book=self.book, source=self.comicvine_source).first()
    ...
    def test_save_cover_from_url_http_error(self, mock_requests_get):
        ...
        _save_cover_from_url(self.book, "https://invalid-url.com/image.jpg", self.comicvine_source)

        # No cover should be saved
        book_cover_count = BookCover.objects.filter(book=self.book, source=self.comicvine_source).count()
...
class ComicVineIntegrationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.scan_folder = create_test_scan_folder(name="Test Scan Folder")
        self.book = create_test_book_with_file(file_path="/test/spider-man-001.cbz", file_format="cbz", file_size=50000000, scan_folder=self.scan_folder)
    ...
    def test_query_comicvine_metadata_success(self, mock_api_class):
        ...
        mock_api.search_volumes.return_value = [{"id": 123, "name": "Amazing Spider-Man", "start_year": 1963, "publisher": {"name": "Marvel Comics"}}]

        # Mock search issues response
        mock_api.search_issues.return_value = [
            {"id": 789, "name": "Origin", "issue_number": "1", "cover_date": "1962-08-01", "person_credits": [{"name": "Stan Lee", "role": "writer"}]}
        ]
    ...
    def test_query_comicvine_metadata_no_specific_issue(self, mock_api_class):
        ...
        mock_api.search_volumes.return_value = [{"id": 123, "name": "Amazing Spider-Man", "start_year": 1963}]
...
class ComicVineErrorHandlingTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(
            file_path="/test/test.cbz", file_format="cbz", file_size=1000000, scan_folder=self.scan_folder, title=None  # Don't create a default title for ComicVine tests
        )
    ...
    def test_save_issue_metadata_invalid_date(self):
        ...
        issue_data = {"name": "Test Issue", "cover_date": "invalid-date-format"}
        ...
        year = BookMetadata.objects.filter(book=self.book, field_name="publication_year").first()
    ...
    def test_save_creators_missing_name(self):
        ...
        person_credits = [{"role": "writer"}, {"name": "Valid Creator", "role": "artist"}]  # Missing name
    ...
    def test_save_cover_from_url_permission_error(self, mock_requests_get):
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp:
            ...
            # Should not raise exception
            _save_cover_from_url(self.book, "https://example.com/cover.jpg", self.comicvine_source)
...
class ComicVinePerformanceTests(TestCase):
    ...
    def test_large_creator_list_handling(self):
        ...
        book = create_test_book_with_file(file_path="/test/test.cbz", file_format="cbz", file_size=1000000, scan_folder=scan_folder)

        # Create a large list of creators
        large_creator_list = [{"name": f"Creator {i}", "role": "contributor"} for i in range(100)]
    ...
    def test_metadata_deduplication(self):
        ...
        volume_data = {"name": "Test Comic", "start_year": 2020, "publisher": {"name": "Test Publisher"}}
        ...
        year_count = BookMetadata.objects.filter(book=book, field_name="publication_year").count()
