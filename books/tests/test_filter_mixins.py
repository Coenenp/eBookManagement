...
class BookFilterMixinTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        """Set up test data"""
        super().setUp()
        self.scan_folder = ScanFolder.objects.create(path=self.temp_dir, name="Filter Test Folder", language="en")

        # Create test books using helper function
        self.epub_book = create_test_book_with_file(file_path="/test/filter/book1.epub", file_format="epub", file_size=1024000, scan_folder=self.scan_folder)

        self.pdf_book = create_test_book_with_file(file_path="/test/filter/book2.pdf", file_format="pdf", file_size=2048000, scan_folder=self.scan_folder)

        self.mobi_book = create_test_book_with_file(file_path="/test/filter/book3.mobi", file_format="mobi", file_size=1536000, scan_folder=self.scan_folder)

        # Create metadata with different confidence levels
        FinalMetadata.objects.create(book=self.epub_book, final_title="High Confidence Book", final_author="Test Author 1", overall_confidence=0.9, is_reviewed=True, language="en")

        FinalMetadata.objects.create(
            book=self.pdf_book, final_title="Medium Confidence Book", final_author="Test Author 2", overall_confidence=0.6, is_reviewed=False, language="en"
        )

        FinalMetadata.objects.create(book=self.mobi_book, final_title="Low Confidence Book", final_author="Test Author 3", overall_confidence=0.3, is_reviewed=False, language="en")
    ...
    def test_apply_search_filters_combined_filters(self):
        ...
        view = MockView()
        queryset = Book.objects.all()
        search_params = {"search_query": "Test Author", "file_format": "epub", "confidence_level": "high"}
        filtered_queryset = view.apply_search_filters(queryset, search_params)
    ...
    def test_apply_search_filters_empty_string_values(self):
        ...
        view = MockView()
        queryset = Book.objects.all()
        search_params = {"search_query": "", "language": "", "file_format": "", "confidence_level": ""}
        filtered_queryset = view.apply_search_filters(queryset, search_params)
    ...
    def test_apply_search_filters_language_filter(self):
        ...
        try:
            french_folder = ScanFolder.objects.create(path=french_dir, name="French Folder", language="fr")

            french_book = create_test_book_with_file(file_path=f"{french_dir}/livre.epub", file_format="epub", file_size=1000000, scan_folder=french_folder)

            FinalMetadata.objects.create(book=french_book, final_title="Livre Français", final_author="Auteur Français", overall_confidence=0.8, language="fr")
    ...
    def test_apply_search_filters_confidence_boundaries(self):
        ...
        boundary_book_1 = create_test_book_with_file(file_path=f"{self.temp_dir}/boundary1.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        boundary_book_2 = create_test_book_with_file(file_path=f"{self.temp_dir}/boundary2.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        # Exactly 0.8 confidence (high boundary)
        FinalMetadata.objects.create(book=boundary_book_1, final_title="Exactly High Boundary", final_author="Boundary Author 1", overall_confidence=0.8)

        # Exactly 0.5 confidence (medium/low boundary)
        FinalMetadata.objects.create(book=boundary_book_2, final_title="Exactly Medium Boundary", final_author="Boundary Author 2", overall_confidence=0.5)
    ...
    def test_apply_search_filters_with_none_values(self):
        ...
        minimal_book = create_test_book_with_file(file_path=f"{self.temp_dir}/minimal.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        FinalMetadata.objects.create(
            book=minimal_book, final_title="Minimal Book", final_author="Unknown Author", overall_confidence=None  # Provide required field  # This can be None
        )
    ...
    def test_apply_search_filters_or_conditions(self):
        ...
        author_match_book = create_test_book_with_file(file_path=f"{self.temp_dir}/author_match.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        FinalMetadata.objects.create(book=author_match_book, final_title="Different Title", final_author="Unique Search Term Author", overall_confidence=0.7)
...
class BookFilterMixinEdgeCaseTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.scan_folder = ScanFolder.objects.create(path=self.temp_dir, name="Edge Test Folder")
    ...
    def test_apply_search_filters_no_metadata(self):
        ...
        create_test_book_with_file(file_path=f"{self.temp_dir}/no_metadata.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
    ...
    def test_apply_search_filters_performance_with_large_dataset(self):
        ...
        for i in range(25):  # Reduced number for testing
            book = create_test_book_with_file(file_path=f"{self.temp_dir}/book_{i}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
            books.append(book)

            FinalMetadata.objects.create(book=book, final_title=f"Performance Test Book {i}", final_author=f"Author {i % 10}", overall_confidence=0.5 + (i % 5) * 0.1)
    ...
    def test_apply_search_filters_database_query_efficiency(self):
        ...
        # Create test data
        for i in range(10):
            book = create_test_book_with_file(file_path=f"{self.temp_dir}/efficient_{i}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

            FinalMetadata.objects.create(book=book, final_title=f"Efficiency Test {i}", final_author="Test Author", overall_confidence=0.8)
...
class BookFilterMixinIntegrationTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user = User.objects.create_user(username="filtertest", password="testpass123")

        self.scan_folder = ScanFolder.objects.create(path=self.temp_dir, name="Integration Test Folder")

        # Create books with rich metadata for integration testing
        self.books_data = [
            {"file_path": "/test/integration/scifi.epub", "title": "Science Fiction Novel", "author": "Isaac Asimov", "format": "epub", "confidence": 0.9},
            {"file_path": "/test/integration/fantasy.pdf", "title": "Fantasy Adventure", "author": "J.R.R. Tolkien", "format": "pdf", "confidence": 0.7},
            {"file_path": "/test/integration/mystery.mobi", "title": "Mystery Thriller", "author": "Agatha Christie", "format": "mobi", "confidence": 0.4},
        ]

        self.books = []
        for book_data in self.books_data:
            book = create_test_book_with_file(
                file_path=f"{self.temp_dir}/{book_data['format']}_book.{book_data['format']}", file_format=book_data["format"], file_size=1000000, scan_folder=self.scan_folder
            )

            FinalMetadata.objects.create(book=book, final_title=book_data["title"], final_author=book_data["author"], overall_confidence=book_data["confidence"])
    ...
    def test_mixin_with_ordering(self):
        ...
        if len(ordered_results) > 1:
            self.assertLessEqual(ordered_results[0].id, ordered_results[1].id)
    ...
    def test_mixin_with_pagination(self):
        ...
        for i in range(25):
            book = create_test_book_with_file(file_path=f"{self.temp_dir}/paginated_{i}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

            FinalMetadata.objects.create(book=book, final_title=f"Paginated Book {i}", final_author="Pagination Author", overall_confidence=0.8)

        view = MockView()

        queryset = Book.objects.all()
        search_params = {"search_query": "Paginated", "confidence_level": "high"}
        filtered_queryset = view.apply_search_filters(queryset, search_params)
