...
class DashboardViewTests(TestCase):
    ...
    def create_test_books(self):
        ...
        self.book2 = create_test_book_with_file(
            file_path="/test/book2.epub", file_format="epub", file_size=800000, is_placeholder=False, scan_folder=self.scan_folder, first_scanned=timezone.now() - timedelta(days=3)
        )
        ...
        self.book3 = create_test_book_with_file(
            file_path="/test/book3.pdf", file_format="pdf", file_size=500000, is_placeholder=False, scan_folder=self.scan_folder, first_scanned=timezone.now() - timedelta(days=1)
        )
        ...
        self.book4 = create_test_book_with_file(file_path="/test/comic1.cbr", file_format="cbr", file_size=50000000, is_placeholder=False, scan_folder=self.scan_folder)
        ...
        self.book5 = create_test_book_with_file(file_path="/test/placeholder.epub", file_format="epub", file_size=0, is_placeholder=True, scan_folder=self.scan_folder)

        # Book 6: Corrupted book
        self.book6 = create_test_book_with_file(file_path="/test/corrupted.epub", file_format="epub", file_size=1000, is_corrupted=True, scan_folder=self.scan_folder)

        # Book 7: Duplicate book
        self.book7 = create_test_book_with_file(file_path="/test/duplicate.epub", file_format="epub", file_size=1000000, is_duplicate=True, scan_folder=self.scan_folder)

        # Create book series relationships
        BookSeries.objects.create(book=self.book1, series=self.series1, series_number="1", is_active=True, source=self.manual_source)
        BookSeries.objects.create(book=self.book4, series=self.series2, series_number="", is_active=True, source=self.epub_source)  # No series number

        # Create scan logs for recent activity
        ScanLog.objects.create(scan_folder=self.scan_folder, timestamp=timezone.now() - timedelta(days=1), books_found=3, books_processed=3, errors_count=0)
    ...
    def test_get_context_data_percentages(self):
        ...
        percentage_fields = ["completion_percentage", "author_percentage", "cover_percentage", "isbn_percentage", "series_percentage", "review_percentage"]
    ...
    def test_get_incomplete_series_count(self):
        ...
        for i, num in enumerate(["1", "3", "5"], 1):
            book = create_test_book_with_file(file_path=f"/test/series{i}.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder)
            FinalMetadata.objects.create(book=book)
            BookSeries.objects.create(book=book, series=series_with_gaps, series_number=num, is_active=True, source=self.epub_source)
    ...
    def test_prepare_chart_data(self):
        ...
        # Mock format stats
        format_stats = [{"files__file_format": "epub", "count": 2}, {"files__file_format": "pdf", "count": 1}, {"files__file_format": "cbr", "count": 1}]
        ...
        # Mock issue stats
        issue_stats = {"missing_covers": 2, "missing_authors": 1, "needs_review": 2, "low_confidence": 1, "missing_isbn": 3}
    ...
    def test_dashboard_with_extreme_values(self):
        ...
        extreme_book = create_test_book_with_file(file_path="/test/extreme.epub", file_format="epub", file_size=999999999, scan_folder=self.scan_folder)
        FinalMetadata.objects.create(book=extreme_book, final_title="Extreme Book", overall_confidence=1.0, completeness_score=0.0)  # Maximum confidence  # Minimum completeness
    ...
    def test_recent_activity_time_boundary(self, mock_now):
        ...
        create_test_book_with_file(
            file_path="/test/week_ago.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder, first_scanned=fixed_now - timedelta(days=7)
        )

        # Create a book 8 days ago (should be excluded)
        create_test_book_with_file(file_path="/test/old.epub", file_format="epub", file_size=1000000, scan_folder=self.scan_folder, first_scanned=fixed_now - timedelta(days=8))
...
class DashboardViewEdgeCaseTests(TestCase):
    ...
    def test_dashboard_with_malformed_series_numbers(self):
        ...
        for i, invalid_number in enumerate(["abc", "N/A", "", None]):
            book = create_test_book_with_file(file_path=f"/test/book{i}.epub", file_format="epub", file_size=1000000, scan_folder=scan_folder)
            FinalMetadata.objects.create(book=book)
            BookSeries.objects.create(book=book, series=series, series_number=invalid_number, is_active=True, source=DataSource.objects.get_or_create(name="Test")[0])
    ...
    def test_dashboard_performance_with_large_dataset(self):
        ...
        metadata_objects = [
            FinalMetadata(book=book, final_title=f"Book {i}", overall_confidence=0.5 + (i % 5) * 0.1, completeness_score=0.3 + (i % 7) * 0.1) for i, book in enumerate(books)
        ]
    ...
    def test_dashboard_with_null_values(self):
        ...
        book = create_test_book_with_file(file_path="/test/null_book.epub", file_format="epub", file_size=1000000, scan_folder=scan_folder)

        # Create metadata with null values
        FinalMetadata.objects.create(
            book=book, final_title=None, final_author=None, isbn=None, publication_year=None, language=None, description=None, overall_confidence=None, completeness_score=None
        )
