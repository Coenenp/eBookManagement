...
class PDFExtractorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(
            file_path=os.path.join(self.temp_dir, "test.pdf"), file_format="pdf", file_size=1024000, scan_folder=self.scan_folder, title=None  # Don't auto-create title
        )

        # Ensure PDF_INTERNAL data source exists
        self.pdf_source, created = DataSource.objects.get_or_create(name=DataSource.PDF_INTERNAL, defaults={"trust_level": 0.8})
    ...
    def test_extract_complete_metadata(self, mock_pdf_reader):
        ...
        creator_metadata = BookMetadata.objects.get(book=self.book, field_name="creator")
    ...
    def test_extract_duplicate_title_handling(self, mock_pdf_reader):
        ...
        existing_title = BookTitle.objects.create(book=self.book, title="Existing Title", source=self.pdf_source, confidence=0.5)
    ...
    def test_extract_duplicate_creator_handling(self, mock_pdf_reader):
        ...
        existing_creator = BookMetadata.objects.create(book=self.book, field_name="creator", field_value="Existing Creator", source=self.pdf_source, confidence=0.5)
        ...
        creators = BookMetadata.objects.filter(book=self.book, field_name="creator")
