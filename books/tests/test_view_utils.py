...
class StandardAjaxResponseMixinTests(TestCase):
    ...
    def test_success_response_with_extra_data(self):
        ...
        response = self.mixin.success_response("Operation completed", book_id=123, count=5)
...
class BookAjaxViewMixinTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(file_path=f"{self.temp_dir}/book.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder)
...
class StandardPaginationMixinTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(50):
            create_test_book_with_file(file_path=f"{self.temp_dir}/book{i}.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder)
...
class BookFilterMixinTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book1 = create_test_book_with_file(file_path=f"{self.temp_dir}/book1.epub", file_format="epub", file_size=1024, scan_folder=self.scan_folder)

        self.book2 = create_test_book_with_file(file_path=f"{self.temp_dir}/book2.pdf", file_format="pdf", file_size=2048, scan_folder=self.scan_folder)

        # Create FinalMetadata for books
        self.metadata1 = FinalMetadata.objects.create(book=self.book1, final_title="Test Book One", final_author="Author One", language="en", is_reviewed=True)
        ...
        self.metadata2 = FinalMetadata.objects.create(book=self.book2, final_title="Test Book Two", final_author="Author Two", language="fr", is_reviewed=False)
        # Set fields after creation to avoid auto-update override
        FinalMetadata.objects.filter(book=self.book2).update(overall_confidence=0.3, final_title="Test Book Two", final_author="Author Two", language="fr")
    ...
    def test_apply_search_filters_combined(self):
        ...
        queryset = Book.objects.all()
        search_params = {"file_format": "epub", "language": "en", "confidence_level": "high"}
        filtered = self.mixin.apply_search_filters(queryset, search_params)
...
class AjaxDecoratorTests(TestCase):
    ...
    def test_standard_ajax_handler_json_parsing(self):
        ...
        request = self.factory.post("/test/", data=json.dumps(request_data), content_type="application/json")
        response = test_view(request)
