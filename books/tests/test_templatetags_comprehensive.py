...
class BookExtrasTemplateTagsTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "test_book.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
        ...
        self.final_metadata = FinalMetadata.objects.create(
            book=self.book, final_title="Test Title", final_author="Test Author", final_cover_path="/test/cover.jpg", is_reviewed=True  # Prevent auto-updating
        )
    ...
    def test_process_cover_for_display_local_file(self, mock_exists, mock_encode):
        ...
        cover_path, is_url, base64_image = book_extras._process_cover_for_display("/local/path/cover.jpg", self.book, False)
    ...
    def test_process_cover_for_display_url(self, mock_download):
        ...
        cover_path, is_url, base64_image = book_extras._process_cover_for_display("http://example.com/cover.jpg", self.book, False)
    ...
    def test_process_cover_for_display_empty_path(self):
        ...
        cover_path, is_url, base64_image = book_extras._process_cover_for_display("", self.book, False)
    ...
    def test_book_cover_with_cover_data(self, mock_process):
        ...
        self.book._cover_data = {"cover_path": "/test/cover.jpg", "cover_base64": "existing_base64"}
    ...
    def test_book_cover_no_finalmetadata(self):
        ...
        book_no_meta = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_meta.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
    ...
    def test_safe_finalmetadata_no_metadata(self):
        ...
        book_no_meta = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_meta2.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
    ...
    def test_has_finalmetadata_false(self):
        ...
        book_no_meta = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_meta3.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
...
class CustomFiltersTemplateTagsTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        BookTitle.objects.create(book=self.book, title="Test Title", source=self.data_source, confidence=1.0)
        BookAuthor.objects.create(book=self.book, author=author, source=self.data_source, confidence=1.0, is_main_author=True)

        self.final_metadata = FinalMetadata.objects.create(book=self.book, final_title="Test Title", final_author="Test Author", is_reviewed=True)  # Prevent auto-updating
    ...
    def test_get_display_title_fallback_to_titles(self):
        ...
        book_no_final = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_final.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)

        filename_source, _ = DataSource.objects.get_or_create(name=DataSource.INITIAL_SCAN, defaults={"trust_level": 0.5})
        BookTitle.objects.create(book=book_no_final, title="Title from BookTitle", source=filename_source, confidence=0.9)
    ...
    def test_get_display_title_fallback_to_filename(self):
        ...
        book_no_meta = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "fallback_book.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
    ...
    def test_get_display_author_fallback(self):
        ...
        book_no_final = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_final_author.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)

        from books.models import BookAuthor

        author = Author.objects.create(name="Fallback Author")
        epub_source, _ = DataSource.objects.get_or_create(name=DataSource.EPUB_INTERNAL, defaults={"trust_level": 0.7})
        BookAuthor.objects.create(book=book_no_final, author=author, source=epub_source, confidence=0.8)
    ...
    def test_get_display_author_no_author(self):
        ...
        book_no_author = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "no_author.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)
...
class TemplateIntegrationTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        self.data_source, _ = DataSource.objects.get_or_create(name="Test Source", defaults={"trust_level": 0.8})
        self.scan_folder = ScanFolder.objects.create(path=self.temp_dir)
        self.book = create_test_book_with_file(file_path=os.path.join(self.temp_dir, "integration_test.epub"), file_size=1000, file_format="epub", scan_folder=self.scan_folder)

        # Create source data for auto-update to pull from
        author = Author.objects.create(name="Test Author")
        BookTitle.objects.create(book=self.book, title="Integration Test Book", source=self.data_source, confidence=1.0)
        BookAuthor.objects.create(book=self.book, author=author, source=self.data_source, confidence=1.0, is_main_author=True)

        self.final_metadata = FinalMetadata.objects.create(
            book=self.book, final_title="Integration Test Book", final_author="Test Author", language="en", is_reviewed=True  # Prevent auto-updating
        )
