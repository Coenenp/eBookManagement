...
class WorkingAITests(TestCase):
    ...
    def test_confidence_evaluation(self):
        ...
        high_conf = {"title": ("Great Title", 0.9), "author": ("Famous Author", 0.8), "series": ("Best Series", 0.85), "volume": ("1", 0.7)}
        self.assertTrue(recognizer.is_prediction_confident(high_conf))

        # Low confidence predictions
        low_conf = {"title": ("Maybe Title", 0.3), "author": ("Unknown Author", 0.2), "series": ("", 0.1), "volume": ("", 0.1)}
...
class AIFeedbackTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = create_test_book_with_file(file_path="/test/test_book.epub", scan_folder=scan_folder)
    ...
    def test_feedback_accuracy_scores(self):
        ...
        ratings_and_scores = [(1, 0.0), (3, 0.5), (5, 1.0)]  # Poor  # Good  # Excellent
