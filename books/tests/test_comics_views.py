...
class ComicsViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.data_source, _ = DataSource.objects.get_or_create(name="Test Comics Source", defaults={"trust_level": 0.8})

        # Create test books with comic formats
        self.comic_book1 = create_test_book_with_file(file_path="/test/comic1.cbr", file_size=5000000, file_format="cbr", scan_folder=self.scan_folder, content_type="comic")

        self.comic_book2 = create_test_book_with_file(file_path="/test/comic2.cbz", file_size=7000000, file_format="cbz", scan_folder=self.scan_folder, content_type="comic")

        self.comic_book3 = create_test_book_with_file(file_path="/test/comic3.cbr", file_size=6000000, file_format="cbr", scan_folder=self.scan_folder, content_type="comic")

        # Create a non-comic book
        self.regular_book = create_test_book_with_file(file_path="/test/book.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder, content_type="ebook")

        # Create FinalMetadata for comics
        self.final_meta1 = FinalMetadata.objects.create(
            book=self.comic_book1, final_title="Batman: Year One", final_author="Frank Miller", final_series="Batman", final_series_number="1", is_reviewed=True
        )

        self.final_meta2 = FinalMetadata.objects.create(
            book=self.comic_book2, final_title="Batman: The Dark Knight Returns", final_author="Frank Miller", final_series="Batman", final_series_number="2", is_reviewed=True
        )

        self.final_meta3 = FinalMetadata.objects.create(
            book=self.comic_book3, final_title="Standalone Comic", final_author="Alan Moore", final_series="", final_series_number="", is_reviewed=True  # No series
        )

        # Create FinalMetadata for regular book (not a comic)
        self.final_meta_book = FinalMetadata.objects.create(book=self.regular_book, final_title="Regular Book", final_author="Author Name", is_reviewed=True)
...
class ComicsMainViewTests(ComicsViewsTestCase):
    ...
    def test_comics_count_excludes_empty_formats(self):
        ...
        empty_comic = create_test_book_with_file(
            file_path="/test/empty.epub", file_size=1000000, file_format="epub", scan_folder=self.scan_folder, content_type="ebook"  # Not a comic format (ebook format)
        )
        FinalMetadata.objects.create(book=empty_comic, final_title="Non-Comic Book", is_reviewed=True)
...
class ComicsAjaxListTests(ComicsViewsTestCase):
    ...
    def test_comics_ajax_list_book_sorting(self):
        ...
        comic_book4 = create_test_book_with_file(file_path="/test/comic4.cbr", file_size=8000000, file_format="cbr", scan_folder=self.scan_folder, content_type="comic")

        FinalMetadata.objects.create(
            book=comic_book4, final_title="Batman: The Killing Joke", final_author="Alan Moore", final_series="Batman", final_series_number="1.5", is_reviewed=True
        )
    ...
    def test_comics_ajax_list_series_sorting(self):
        ...
        comic_book5 = create_test_book_with_file(file_path="/test/comic5.cbz", file_size=5500000, file_format="cbz", scan_folder=self.scan_folder, content_type="comic")

        FinalMetadata.objects.create(
            book=comic_book5, final_title="Amazing Spider-Man #1", final_author="Stan Lee", final_series="Amazing Spider-Man", final_series_number="1", is_reviewed=True
        )
...
class ComicsIntegrationTests(ComicsViewsTestCase):
    ...
    def test_comics_with_mixed_metadata_sources(self):
        ...
        BookSeries.objects.create(book=self.comic_book1, series=series, series_number="1", source=self.data_source, confidence=0.9, is_active=True)
