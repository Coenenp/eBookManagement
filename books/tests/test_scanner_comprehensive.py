...
class ScannerEngineTests(TestCase):
    ...
    def setUp(self):
        ...
        self.scan_folder = ScanFolder.objects.create(name="Test Scan Folder", path=self.temp_dir, is_active=True, language="en")

        self.source, _ = DataSource.objects.get_or_create(name="Scanner Test Source", defaults={"priority": 1})

        # Create required DataSource for scanner
        self.initial_scan_source, _ = DataSource.objects.get_or_create(name=DataSource.INITIAL_SCAN, defaults={"priority": 0})

        # Create file_scanner DataSource that the scanner uses
        self.file_scanner_source, _ = DataSource.objects.get_or_create(name="file_scanner", defaults={"priority": 1})
    ...
    def test_scan_folder_with_comics(self, mock_scan_directory):
        ...
        test_files = [{"name": "comic1.cbz", "size": 5120000}, {"name": "comic2.cbr", "size": 4096000}, {"name": "series/issue_001.cbz", "size": 6144000}]
    ...
    def test_scan_progress_tracking(self, mock_collect_files, mock_scan_directory):
        ...
        test_files = [{"name": f"book_{i:03d}.epub", "size": 1024000} for i in range(20)]
    ...
    def test_duplicate_file_handling(self, mock_scan_directory):
        ...
        test_files = [{"name": "book.epub", "size": 1024000}]
    ...
    def test_cancellation_handling(self, mock_scan_directory):
        ...
        test_files = [{"name": f"book_{i:03d}.epub", "size": 1024000} for i in range(10)]  # Reduced number for testing
...
class ScannerErrorRecoveryTests(TestCase):
    ...
    def test_scanner_memory_usage_monitoring(self, mock_scan_directory):
        ...
        test_files = [{"name": f"book_{i:04d}.epub", "size": 1024} for i in range(10)]  # Reduced number for testing
