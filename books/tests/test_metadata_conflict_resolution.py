...
class MetadataConflictResolutionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.google_books_source, _ = DataSource.objects.get_or_create(name=DataSource.GOOGLE_BOOKS, defaults={"trust_level": 0.8})

        self.manual_source, _ = DataSource.objects.get_or_create(name=DataSource.MANUAL, defaults={"trust_level": 0.95})
    ...
    def test_highest_confidence_metadata_wins(self):
        ...
        BookTitle.objects.create(book=self.book, title="Low Confidence Title", source=self.initial_scan_source, confidence=0.3, is_active=True)

        BookTitle.objects.create(book=self.book, title="High Confidence Title", source=self.google_books_source, confidence=0.9, is_active=True)

        BookTitle.objects.create(book=self.book, title="Medium Confidence Title", source=self.epub_source, confidence=0.6, is_active=True)
    ...
    def test_manual_entry_always_wins(self):
        ...
        BookAuthor.objects.create(book=self.book, author=api_author, source=self.google_books_source, confidence=0.8, is_main_author=True, is_active=True)

        # Create manual entry with higher confidence (current implementation uses confidence only)
        BookAuthor.objects.create(book=self.book, author=manual_author, source=self.manual_source, confidence=0.95, is_main_author=True, is_active=True)  # Higher than API
    ...
    def test_source_trust_level_breaks_confidence_ties(self):
        ...
        BookPublisher.objects.create(book=self.book, publisher=initial_publisher, source=self.initial_scan_source, confidence=0.7, is_active=True)  # trust_level: 0.2

        BookPublisher.objects.create(book=self.book, publisher=epub_publisher, source=self.epub_source, confidence=0.7, is_active=True)  # trust_level: 0.7  # Same confidence
    ...
    def test_combined_confidence_calculation(self):
        ...
        BookMetadata.objects.create(book=self.book, field_name="isbn", field_value="978-0123456789", source=self.epub_source, confidence=0.8, is_active=True)  # trust_level: 0.7
    ...
    def test_partial_metadata_resolution(self):
        ...
        BookTitle.objects.create(book=self.book, title="Available Title", source=self.epub_source, confidence=0.8, is_active=True)

        BookMetadata.objects.create(book=self.book, field_name="language", field_value="en", source=self.epub_source, confidence=0.9, is_active=True)
    ...
    def test_conflicting_isbn_resolution(self):
        ...
        BookMetadata.objects.create(
            book=self.book, field_name="isbn", field_value="978-1111111111", source=self.initial_scan_source, confidence=0.4, is_active=True  # From filename parsing
        )

        BookMetadata.objects.create(book=self.book, field_name="isbn", field_value="978-2222222222", source=self.epub_source, confidence=0.8, is_active=True)  # From EPUB metadata

        BookMetadata.objects.create(
            book=self.book, field_name="isbn", field_value="978-3333333333", source=self.google_books_source, confidence=0.85, is_active=True  # From external API
        )
    ...
    def test_author_relationship_conflict_resolution(self):
        ...
        BookAuthor.objects.create(book=self.book, author=author1, source=self.initial_scan_source, confidence=0.3, is_main_author=True, is_active=True)

        BookAuthor.objects.create(book=self.book, author=author2, source=self.epub_source, confidence=0.7, is_main_author=True, is_active=True)

        BookAuthor.objects.create(book=self.book, author=author3, source=self.google_books_source, confidence=0.9, is_main_author=True, is_active=True)
    ...
    def test_reviewed_metadata_prevents_overwrite(self):
        ...
        final_metadata = FinalMetadata.objects.create(book=self.book, final_title="Reviewed Title", final_author="Reviewed Author", is_reviewed=True)  # Marked as reviewed

        # Add new high confidence metadata that would normally win
        BookMetadata.objects.create(book=self.book, field_name="title", field_value="New High Confidence Title", source=self.manual_source, confidence=0.99)
    ...
    def test_conflict_resolution_behavior(self):
        ...
        BookTitle.objects.create(book=self.book, title="Title A", source=self.initial_scan_source, confidence=0.5, is_active=True)

        BookTitle.objects.create(book=self.book, title="Title B", source=self.epub_source, confidence=0.8, is_active=True)
