...
class UserProfileTests(TestCase):
    ...
    def test_user_preferences_creation(self):
        ...
        try:
            ...
            preferences = UserProfile.objects.create(user=self.user, theme="flatly", items_per_page=25)
            self.assertIsNotNone(preferences)

    def test_user_preferences_defaults(self):
        """Test default values for user preferences."""
        preferences, created = UserProfile.objects.get_or_create(
            user=self.user, defaults={"theme": "flatly", "items_per_page": 25, "show_covers_in_list": True, "default_view_mode": "table"}
        )
    ...
    def test_update_user_preferences(self):
        ...
        preferences, created = UserProfile.objects.get_or_create(user=self.user, defaults={"theme": "flatly"})
...
class ThemeSettingsTests(TestCase):
    ...
    def test_theme_settings_ajax_endpoint(self):
        ...
        response = self.client.post(url, data=json.dumps(theme_data), content_type="application/json")
    ...
    def test_theme_reset_to_defaults(self):
        ...
        UserProfile.objects.update_or_create(user=self.user, defaults={"theme": "darkly"})
...
class LanguagePreferencesTests(TestCase):
    ...
    def test_language_preference_setting(self):
        ...
        response = self.client.post(url, data=json.dumps(language_data), content_type="application/json")
    ...
    def test_language_detection_from_browser(self):
        ...
        response = self.client.get(reverse("books:book_list"), HTTP_ACCEPT_LANGUAGE="es-ES,es;q=0.9,en;q=0.8")
...
class DisplayOptionsTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test books
        for i in range(30):
            create_test_book_with_file(file_path=f"/library/display_test_{i+1}.epub", file_format="epub", scan_folder=self.scan_folder)

    def test_items_per_page_setting(self):
        """Test items per page display setting."""
        # Set books per page to 10
        UserProfile.objects.update_or_create(user=self.user, defaults={"items_per_page": 10})
    ...
    def test_cover_display_toggle(self):
        ...
        UserProfile.objects.update_or_create(user=self.user, defaults={"show_covers_in_list": True})
        ...
        UserProfile.objects.update_or_create(user=self.user, defaults={"show_covers_in_list": False})
    ...
    def test_grid_vs_list_view(self):
        ...
        # Test grid view
        UserProfile.objects.update_or_create(user=self.user, defaults={"default_view_mode": "grid"})

        response = self.client.get(reverse("books:book_list"))
        self.assertEqual(response.status_code, 200)

        # Test list view
        UserProfile.objects.update_or_create(user=self.user, defaults={"default_view_mode": "table"})
    ...
    def test_ajax_update_display_options(self):
        ...
        display_data = {"items_per_page": 25, "show_covers_in_list": True, "default_view_mode": "grid"}

        response = self.client.post(url, data=json.dumps(display_data), content_type="application/json")
...
class CachingBehaviorTests(TestCase):
    ...
    def test_user_preference_caching(self):
        ...
        preferences = UserProfile.objects.create(user=self.user, theme="darkly")
    ...
    def test_theme_css_caching(self):
        ...
        theme_data = {"theme": "dark", "accent_color": "#007bff"}
    ...
    def test_cache_invalidation_on_preference_update(self):
        ...
        preferences = UserProfile.objects.create(user=self.user, theme="light")
...
class PersonalizationFeaturesTests(TestCase):
    ...
    def test_custom_dashboard_layout(self):
        ...
        layout_data = {"widgets": ["recent_books", "statistics", "quick_actions"], "widget_order": [1, 2, 3], "show_recommendations": True}

        url = reverse("books:ajax_update_dashboard_layout")

        response = self.client.post(url, data=json.dumps(layout_data), content_type="application/json")
    ...
    def test_favorite_genres_tracking(self):
        ...
        genre_data = {"favorites": ["Science Fiction", "Fantasy", "Mystery"], "hidden": ["Romance", "Horror"]}

        url = reverse("books:ajax_update_favorite_genres")

        response = self.client.post(url, data=json.dumps(genre_data), content_type="application/json")
    ...
    def test_reading_progress_tracking(self):
        ...
        progress_data = {"book_id": 1, "progress_percentage": 45, "current_page": 123, "total_pages": 273, "reading_time": 7200}  # seconds

        url = reverse("books:ajax_update_reading_progress")

        response = self.client.post(url, data=json.dumps(progress_data), content_type="application/json")
    ...
    def test_custom_tags_and_labels(self):
        ...
        tag_data = {"book_id": 1, "tags": ["to-read", "favorites", "recommended"], "custom_labels": ["Award Winner", "Book Club Pick"]}

        url = reverse("books:ajax_update_custom_tags")

        response = self.client.post(url, data=json.dumps(tag_data), content_type="application/json")
    ...
    def test_import_user_preferences(self):
        ...
        preferences_data = {"theme": "darkly", "items_per_page": 50}

        url = reverse("books:ajax_import_preferences")

        response = self.client.post(url, data=json.dumps({"preferences": preferences_data}), content_type="application/json")
...
class UserSettingsSecurityTests(TestCase):
    ...
    def test_user_cannot_modify_other_preferences(self):
        ...
        malicious_data = {"user_id": self.other_user.id, "theme": "modified_by_attacker"}
    ...
    def test_preference_input_validation(self):
        ...
        malicious_data = {"theme": '<script>alert("xss")</script>', "accent_color": "javascript:alert(1)", "custom_css": 'body { background: url("javascript:alert(1)"); }'}
    ...
    def test_preference_size_limits(self):
        ...
        response = self.client.post(url, data=json.dumps(large_data), content_type="application/json")
...
class ConfigurationIntegrationTests(TestCase):
    ...
    def test_theme_persistence_across_pages(self):
        ...
        # Set dark theme
        UserProfile.objects.update_or_create(user=self.user, defaults={"theme": "dark"})
    ...
    def test_pagination_settings_consistency(self):
        ...
        # Create test books
        for i in range(15):
            create_test_book_with_file(file_path=f"/library/pagination_{i}.epub", file_format="epub", scan_folder=scan_folder)

        # Set books per page to 5
        UserProfile.objects.update_or_create(user=self.user, defaults={"items_per_page": 5})
    ...
    def test_complete_configuration_workflow(self):
        ...
        display_response = self.client.post(reverse("books:ajax_update_display_options"), data=json.dumps({"items_per_page": 10}), content_type="application/json")
