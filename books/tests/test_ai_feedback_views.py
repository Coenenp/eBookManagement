...
class AIFeedbackListViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.ai_source = DataSource.objects.create(name="filename_ai", trust_level=0.7)

        # Create test books with AI predictions
        self.books_with_ai = []
        for i in range(5):
            book = Book.objects.create()
            BookFile.objects.create(book=book, file_path=f"/library/ai_book_{i+1}.epub", file_format="epub")
            ...
            BookMetadata.objects.create(
                book=book, source=self.ai_source, field_name="ai_title", field_value=f"AI Predicted Title {i+1}", confidence=0.5 + (i * 0.1), is_active=True
            )
    ...
    def test_ai_feedback_list_statistics(self):
        ...
        expected_stats = ["total_ai_predictions", "low_confidence", "medium_confidence", "high_confidence", "needs_review", "reviewed"]
    ...
    def test_ai_feedback_list_pagination(self):
        ...
        for i in range(60):  # More than default paginate_by (50)
            book = Book.objects.create()
            BookFile.objects.create(book=book, file_path=f"/library/extra_{i}.epub", file_format="epub")

            FinalMetadata.objects.create(book=book, final_title=f"Extra AI Book {i}", overall_confidence=0.5)

            BookMetadata.objects.create(book=book, source=self.ai_source, field_name="ai_title", field_value=f"AI Title {i}", confidence=0.7, is_active=True)
...
class AIFeedbackDetailViewTests(TestCase):
    ...
    def setUp(self):
        ...
        BookFile.objects.create(book=self.book, file_path="/library/ai_detail_test.epub", file_format="epub")

        self.final_metadata = FinalMetadata.objects.create(
            book=self.book, final_title="AI Detail Test Book", final_author="AI Test Author", final_series="AI Test Series", overall_confidence=0.75, is_reviewed=False
        )

        # Create AI metadata entries
        ai_fields = [("ai_title", "AI Predicted Title"), ("ai_author", "AI Predicted Author"), ("ai_series", "AI Predicted Series"), ("ai_volume", "1")]

        for field_name, field_value in ai_fields:
            BookMetadata.objects.create(book=self.book, source=self.ai_source, field_name=field_name, field_value=field_value, confidence=0.8, is_active=True)
    ...
    def test_ai_feedback_detail_no_metadata(self):
        ...
        BookFile.objects.create(book=book_no_meta, file_path="/library/no_meta.epub", file_format="epub")
...
class AIFeedbackSubmissionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = Book.objects.create()
        BookFile.objects.create(book=self.book, file_path="/library/feedback_test.epub", file_format="epub")

        FinalMetadata.objects.create(book=self.book, final_title="Feedback Test Book", final_author="Test Author", is_reviewed=False)
    ...
    def test_submit_ai_feedback_success(self):
        ...
        feedback_data = {
            "corrections": {"title": "Corrected Title", "author": "Corrected Author", "series": "Corrected Series", "volume": "2"},
            "rating": 4,
    ...
    def test_submit_ai_feedback_updates_metadata(self):
        ...
        feedback_data = {"corrections": {"title": "New Corrected Title", "author": "New Corrected Author"}, "rating": 5}
    ...
    def test_submit_ai_feedback_invalid_book(self):
        ...
        feedback_data = {"corrections": {}, "rating": 3}
...
class AIModelRetrainingTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            book = Book.objects.create()
            BookFile.objects.create(book=book, file_path=f"/library/retrain_{i}.epub", file_format="epub")
    ...
    def test_retrain_ai_models_sufficient_feedback(self):
        ...
        for i, book in enumerate(self.books[:7]):  # 7 feedback entries
            AIFeedback.objects.create(book=book, user=self.user, feedback_rating=4 + (i % 2), needs_retraining=True)  # Ratings 4 or 5
    ...
    def test_retrain_ai_models_insufficient_feedback(self):
        ...
        for book in self.books[:3]:  # Only 3 feedback entries
            AIFeedback.objects.create(book=book, user=self.user, feedback_rating=3, needs_retraining=True)
...
class AIModelStatusTests(TestCase):
    ...
    def test_ai_model_status_success(self, mock_aggregate, mock_recognizer_class):
        ...
        mock_recognizer.get_training_data_stats.return_value = {"total_samples": 1000, "accuracy": 0.85, "last_trained": "2023-12-01"}
        ...
        book = Book.objects.create()
        BookFile.objects.create(book=book, file_path="/library/status_test.epub", file_format="epub")

        AIFeedback.objects.create(book=book, user=self.user, feedback_rating=4, needs_retraining=True)
...
class AIFeedbackIntegrationTests(TestCase):
    ...
    def test_complete_ai_feedback_workflow(self):
        ...
        # Create AI data source
        ai_source = DataSource.objects.create(name="filename_ai", trust_level=0.7)

        # Create book with AI metadata
        book = Book.objects.create()
        BookFile.objects.create(book=book, file_path="/library/workflow_test.epub", file_format="epub")

        # Create BookMetadata so the book shows up in the AI feedback list
        BookMetadata.objects.create(book=book, source=ai_source, field_name="ai_title", field_value="AI Predicted Title", confidence=0.8, is_active=True)
        ...
        final_metadata, created = FinalMetadata.objects.get_or_create(
            book=book,
            defaults={"final_title": "", "final_author": "Test Author", "overall_confidence": 0.6, "is_reviewed": False},  # Empty string will show "Unknown Title" in template
        )
        if not created:
        ...
        # 3. Submit feedback
        feedback_data = {"corrections": {"title": "Corrected Title"}, "rating": 4, "comments": "Good prediction"}

        response = self.client.post(reverse("books:ajax_submit_ai_feedback", kwargs={"book_id": book.id}), data=json.dumps(feedback_data), content_type="application/json")
    ...
    def test_ai_feedback_with_large_dataset(self):
        ...
        for i in range(100):
            book = Book.objects.create()
            BookFile.objects.create(book=book, file_path=f"/library/large_{i}.epub", file_format="epub")
            ...
            BookMetadata.objects.create(book=book, source=ai_source, field_name="ai_title", field_value=f"AI Title {i}", confidence=0.7, is_active=True)
...
class AIFeedbackSecurityTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book = Book.objects.create()
        BookFile.objects.create(book=self.book, file_path="/library/security_test.epub", file_format="epub")
    ...
    def test_ai_feedback_input_validation(self):
        ...
        malicious_data = {"corrections": {"title": '<script>alert("xss")</script>', "author": '"><script>alert("xss")</script>'}, "rating": 4}

        response = self.client.post(url, data=json.dumps(malicious_data), content_type="application/json")
