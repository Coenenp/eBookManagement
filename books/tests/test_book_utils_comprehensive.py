...
class MetadataProcessorTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        self.manual_source, _ = DataSource.objects.get_or_create(name="Manual Entry", defaults={"trust_level": 1.0})
        ...
        self.book = create_test_book_with_file(file_path=test_file_path, file_format="epub", file_size=1000000, scan_folder=self.scan_folder)

        # Create final metadata
        self.final_metadata = FinalMetadata.objects.create(book=self.book, final_title="Original Title", final_author="Original Author", is_reviewed=False)
    ...
    def test_handle_manual_entries_basic_functionality(self):
        ...
        request = self.factory.post("/", {"final_title": "New Title", "manual_entry_final_title": "true", "final_author": "New Author", "manual_entry_final_author": "true"})
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_title": "New Title", "final_author": "New Author"}
    ...
    def test_handle_manual_entries_title_processing(self):
        ...
        request = self.factory.post("/", {"final_title": "__manual__", "manual_title": "Manual Title Entry", "manual_entry_final_title": "true"})
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_title": "__manual__"}
    ...
    def test_handle_manual_entries_author_processing(self):
        ...
        request = self.factory.post("/", {"final_author": "__manual__", "manual_author": "Manual Author Entry", "manual_entry_final_author": "true"})
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_author": "__manual__"}
    ...
    def test_handle_manual_entries_series_processing(self):
        ...
        request = self.factory.post("/", {"final_series": "__manual__", "manual_series": "Manual Series Entry", "manual_entry_final_series": "true"})
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_series": "__manual__"}
    ...
    def test_handle_manual_entries_publisher_processing(self):
        ...
        request = self.factory.post("/", {"final_publisher": "__manual__", "manual_publisher": "Manual Publisher Entry", "manual_entry_final_publisher": "true"})
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_publisher": "__manual__"}
    ...
    def test_handle_manual_entries_metadata_fields(self):
        ...
        form_data = {"publication_year": "2023", "language": "English", "isbn": "9781234567890", "description": "Test description"}
    ...
    def test_handle_manual_entries_existing_data_check(self):
        ...
        BookTitle.objects.create(book=self.book, title="Existing Title", confidence=0.8, is_active=True, source=self.epub_source)

        request = self.factory.post("/", {"final_title": "New Title", "manual_entry_final_title": "false"})  # Explicitly not manual
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_title": "New Title"}
    ...
    def test_handle_manual_entries_empty_values(self):
        ...
        request = self.factory.post("/", {"final_title": "", "manual_entry_final_title": "true", "final_author": "   ", "manual_entry_final_author": "true"})  # Whitespace only
        request.user = self.user
        request.POST = request.POST.copy()

        form_data = {"final_title": "", "final_author": "   "}
    ...
    def test_handle_manual_entries_manual_flag_precedence(self):
        ...
        request = self.factory.post("/", {"final_title": "New Title", "manual_entry_final_title": "true"})  # Explicit manual flag
        request.user = self.user
        request.POST = request.POST.copy()
    ...
    def test_handle_manual_entries_bulk_operations(self):
        ...
        request = self.factory.post("/", {"final_title": "New Title", "manual_entry_final_title": "true"})
        request.user = self.user
        request.POST = request.POST.copy()
    ...
    def test_handle_manual_entries_complex_scenario(self):
        ...
        BookAuthor.objects.create(book=self.book, author=self.author, confidence=0.7, is_active=True, source=self.epub_source)
        # Create existing series data to test the "has existing data" logic
        BookSeries.objects.create(book=self.book, series=self.series, confidence=0.6, is_active=True, source=self.epub_source)
        ...
        form_data = {"final_title": "New Title", "final_author": "__manual__", "final_series": "Existing Series", "publication_year": "2023"}
...
class MetadataProcessorHelperMethodTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        # Create final metadata
        self.final_metadata = FinalMetadata.objects.create(book=self.book, final_title="Original Title", is_reviewed=False)

    def test_handle_manual_title_new_entry(self):
        """Test creating new manual title entry"""
        form_data = {"final_title": "New Manual Title"}

        entry = MetadataProcessor._handle_manual_title(self.book, "final_title", form_data, self.manual_source)
    ...
    def test_handle_manual_author_new_entry(self):
        ...
        entry = MetadataProcessor._handle_manual_author(self.book, "final_author", form_data, self.manual_source)
    ...
    def test_handle_manual_series_new_entry(self):
        ...
        entry = MetadataProcessor._handle_manual_series(self.book, "final_series", form_data, self.manual_source)
    ...
    def test_handle_manual_publisher_new_entry(self):
        ...
        entry = MetadataProcessor._handle_manual_publisher(self.book, "final_publisher", form_data, self.manual_source)
    ...
    def test_handle_manual_metadata_field_new_entry(self):
        ...
        entry = MetadataProcessor._handle_manual_metadata_field(self.book, "publication_year", form_data, self.manual_source)
    ...
    def test_handle_manual_metadata_field_different_types(self):
        ...
        test_cases = [("isbn", "9781234567890"), ("language", "English"), ("description", "A test book description"), ("publication_year", "2023")]

        for field_name, field_value in test_cases:
            with self.subTest(field=field_name):
                form_data = {field_name: field_value}

                entry = MetadataProcessor._handle_manual_metadata_field(self.book, field_name, form_data, self.manual_source)
...
class MetadataProcessorErrorHandlingTests(BaseTestCaseWithTempDir):
    ...
    def test_handle_manual_entries_with_none_values(self):
        ...
        form_data = {"final_title": None, "final_author": None}
    ...
    def test_handle_manual_entries_with_special_characters(self):
        ...
        form_data = {"final_title": "Title with Spëcial Cháracters & Symbols!", "final_author": "Åuthör with ñoñ-ASCII chars 中文"}
...
class MetadataProcessorIntegrationTests(BaseTestCaseWithTempDir):
    ...
    def setUp(self):
        ...
        self.final_metadata = FinalMetadata.objects.create(book=self.book, final_title="Original Title")
    ...
    def test_end_to_end_manual_entry_processing(self):
        ...
        form_data = {"final_title": "Complete Manual Title", "final_author": "__manual__", "final_series": "Complete Manual Series", "publication_year": "2023"}

        # Process the manual entries
        MetadataProcessor.handle_manual_entries(request, self.book, form_data)

        # Verify all entries were created
        self.assertTrue(BookTitle.objects.filter(book=self.book, title="Complete Manual Title", source__name="Manual Entry").exists())
        ...
        self.assertTrue(BookAuthor.objects.filter(book=self.book, author=author, source__name="Manual Entry").exists())
        ...
        self.assertTrue(BookSeries.objects.filter(book=self.book, series=series, source__name="Manual Entry").exists())

        self.assertTrue(BookMetadata.objects.filter(book=self.book, field_name="publication_year", field_value="2023", source__name="Manual Entry").exists())
    ...
    def test_mixed_manual_and_existing_data_processing(self):
        ...
        BookAuthor.objects.create(book=self.book, author=existing_author, source=epub_source, confidence=0.8, is_active=True)
        ...
        form_data = {"final_title": "New Manual Title", "final_author": "Existing Author", "final_series": "__manual__"}

        MetadataProcessor.handle_manual_entries(request, self.book, form_data)

        # Verify manual title was created
        self.assertTrue(BookTitle.objects.filter(book=self.book, title="New Manual Title", source__name="Manual Entry").exists())

        # Verify no new manual author entry was created (existing data exists)
        manual_author_entries = BookAuthor.objects.filter(book=self.book, source__name="Manual Entry")
