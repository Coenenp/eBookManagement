...
class WizardPrePopulationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.wizard = SetupWizard.objects.create(
            ...
            is_completed=True,
            scraper_config={
                'google_books': 'stored_google_api_key_123456789012345',
                'comic_vine': 'stored_comic_vine_key_123456789012345678901',
                'open_library': ''
            },
    ...
    @patch.dict(os.environ, {
        'GOOGLE_BOOKS_API_KEY': 'env_google_123',
        'COMIC_VINE_API_KEY': 'env_comic_456'
    })
    def test_scrapers_pre_population_from_environment(self):
        ...
        user_env = User.objects.create_user(
    ...
    def test_pre_population_priority_database_over_environment(self):
        ...
        with patch.dict(os.environ, {
            'GOOGLE_BOOKS_API_KEY': 'env_key_should_be_ignored'
        }):
            request = self.factory.get('/wizard/scrapers/')
            request.user = self.user
    ...
    def test_pre_population_data_validation(self):
        ...
        SetupWizard.objects.create(
            ...
            scraper_config={
                ...
                'invalid_scraper': 'should_be_ignored'
            }
        )
...
class WizardPrePopulationSecurityTests(TestCase):
    ...
    def test_pre_population_data_isolation(self):
        ...
        SetupWizard.objects.create(
            user=user1,
            scraper_config={'google_books': 'user1_secret_key'}
        )

        SetupWizard.objects.create(
            user=user2,
            scraper_config={'google_books': 'user2_secret_key'}
        )
    ...
    def test_pre_population_sensitive_data_handling(self):
        ...
        SetupWizard.objects.create(
            user=self.user,
            scraper_config={
                'google_books': 'very_sensitive_api_key_12345',
                'comic_vine': 'another_sensitive_key_67890'
            }
        )
        ...
        for scraper in scrapers:
            if 'current_value' in scraper and scraper['current_value']:
                ...
                if len(current_value) > 10:
                    ...
                    self.assertTrue(
                        '*' in current_value or
                        len(current_value) <= 20 or  # Truncated
                        'preview' in scraper  # Has preview field
                    )
