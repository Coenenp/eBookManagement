...
class WizardRequiredMixinTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(username="testuser_wizard", password="testpass123")
        self.client = Client()
...
class WizardWelcomeViewTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(username="testuser_welcome", password="testpass123")
        self.client = Client()
        self.client.force_login(self.user)
...
class WizardFoldersViewTests(TestCase):
    ...
    def test_wizard_folders_view_add_folder_valid(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_folders"), {"folders": [self.temp_dir]})
    ...
    def test_wizard_folders_view_add_folder_invalid_path(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_folders"), {"folders": ["/nonexistent/path"]})
    ...
    def test_wizard_folders_view_permission_check(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_folders"), {"folders": ["/restricted/path"]})
    ...
    def test_wizard_folders_view_ajax_folder_validation(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_validate_folder"), {"path": self.temp_dir}, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
...
class WizardContentTypesViewTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(username="testuser_content", password="testpass123")
        self.client = Client()
        self.client.force_login(self.user)
    ...
    def test_wizard_content_types_view_select_comics(self):
        ...
        response = self.client.post(reverse("books:wizard_content_types"), {"content_type": "comics"})
    ...
    def test_wizard_content_types_view_select_both(self):
        ...
        response = self.client.post(reverse("books:wizard_content_types"), {"content_type": "both"})
    ...
    def test_wizard_content_types_view_invalid_selection(self):
        ...
        response = self.client.post(reverse("books:wizard_content_types"), {"content_type": "invalid_type"})
...
class WizardScrapersViewTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(username="testuser_scrapers", password="testpass123")
        self.client = Client()
        self.client.force_login(self.user)
    ...
    def test_wizard_scrapers_view_enable_scrapers(self):
        ...
        response = self.client.post(reverse("books:wizard_scrapers"), {"enabled_scrapers": ["goodreads", "google_books", "comicvine"]})
    ...
    def test_wizard_scrapers_view_api_key_configuration(self):
        ...
        response = self.client.post(reverse("books:wizard_scrapers"), {"enabled_scrapers": ["goodreads"], "goodreads_api_key": "test_api_key_123"})
    ...
    def test_wizard_scrapers_view_test_api_connection(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            response = self.client.post(reverse("books:ajax_test_connection"), {"scraper": "goodreads", "api_key": "test_key"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
    ...
    def test_wizard_scrapers_view_invalid_api_key(self):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen:
            ...
            response = self.client.post(reverse("books:ajax_test_connection"), {"service": "isbn_lookup"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
    ...
    def test_wizard_scrapers_view_no_scrapers_selected(self):
        ...
        response = self.client.post(reverse("books:wizard_scrapers"), {"enabled_scrapers": []})
...
class WizardCompleteViewTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(username="testuser_complete", password="testpass123")
        self.client = Client()
        self.client.force_login(self.user)
    ...
    def test_wizard_complete_view_start_initial_scan(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.Popen") as mock_popen:
                ...
                response = self.client.post(reverse("books:wizard_complete"), {"start_scan": "true"})
...
class WizardIntegrationTests(TestCase):
    ...
    def test_complete_wizard_flow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            response = self.client.post(reverse("books:wizard_folders"), {"folders": [temp_dir]})  # Submit the same folder that's already in wizard
            self.assertEqual(response.status_code, 302)
            ...
            response = self.client.post(reverse("books:wizard_content_types"), {folder_key: "ebooks"})
            self.assertEqual(response.status_code, 302)
    ...
    def test_wizard_session_persistence(self):
        ...
        # Set content types
        response = self.client.post(reverse("books:wizard_content_types"), {"content_types": ["comics"]})
        self.assertEqual(response.status_code, 302)
    ...
    def test_wizard_navigation_controls(self):
        ...
        steps = ["wizard_welcome", "wizard_folders", "wizard_content_types", "wizard_scrapers", "wizard_complete"]
    ...
    def test_wizard_responsive_design(self):
        ...
        for step in ["wizard_welcome", "wizard_folders", "wizard_content_types"]:
            ...
            self.assertTrue("viewport" in content or "responsive" in content or "col-" in content)  # Bootstrap responsive classes
...
class WizardPerformanceTests(TestCase):
    ...
    def test_wizard_folder_validation_performance(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_validate_folder"), {"folder_path": "/test/performance/path"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest")
    ...
    def test_wizard_large_folder_list_handling(self):
        ...
        try:
            for i in range(50):
                ...
                ScanFolder.objects.create(path=temp_dir, name=f"Performance Test Folder {i}")
    ...
    def test_wizard_session_size_optimization(self):
        ...
        self.client.post(reverse("books:wizard_scrapers"), {"enabled_scrapers": ["goodreads", "google_books", "comicvine"]})
...
class WizardSecurityTests(TestCase):
    ...
    def test_wizard_path_traversal_protection(self):
        ...
        malicious_paths = ["../../../etc/passwd", "..\\..\\windows\\system32", "/etc/shadow", "../../../../var/log"]
    ...
    def test_wizard_input_sanitization(self):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            response = self.client.post(reverse("books:wizard_folders"), {"action": "add_folder", "folder_path": "/safe/path", "folder_name": malicious_name})
