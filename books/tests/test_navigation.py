...
class BookDetailNavigationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.scan_folder = ScanFolder.objects.create(name="Test Folder", path=tempfile.mkdtemp(), language="en")

        # Create test books with proper Book model fields
        # Create books first
        self.book1 = create_test_book_with_file(file_path="/test/book1.epub", file_format="epub", scan_folder=self.scan_folder)

        self.book2 = create_test_book_with_file(file_path="/test/book2.epub", file_format="epub", scan_folder=self.scan_folder)

        self.book3 = create_test_book_with_file(file_path="/test/book3.epub", file_format="epub", scan_folder=self.scan_folder)
    ...
    def test_navigation_with_placeholder_books(self):
        ...
        create_test_book_with_file(file_path="/test/placeholder.epub", file_format="placeholder", scan_folder=self.scan_folder, is_placeholder=True)
    ...
    def test_navigation_with_missing_final_metadata(self):
        ...
        create_test_book_with_file(file_path="/test/book4.epub", file_format="epub", scan_folder=self.scan_folder)
...
class BookDetailNavigationIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create scan folder
        self.scan_folder = ScanFolder.objects.create(path=tempfile.mkdtemp(), language="en")

        # Create multiple books for comprehensive testing
        self.books = []
        for i in range(1, 6):  # Create books 1-5
            book = create_test_book_with_file(file_path=f"/test/book{i}.epub", file_format="epub", scan_folder=self.scan_folder)
            self.books.append(book)

            # Create final metadata
            FinalMetadata.objects.create(book=book, final_title=f"Book {i}", final_author="Test Author", is_reviewed=(i % 2 == 0))  # Even numbered books are reviewed
