...
class RenamingViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create test books
        self.book1 = create_test_book_with_file(file_path="/test/Foundation.epub", file_size=1024000, file_format="epub")

        self.book2 = create_test_book_with_file(file_path="/test/Foundation and Empire.epub", file_size=1536000, file_format="epub")
        ...
        self.data_source, created = DataSource.objects.get_or_create(name=DataSource.INITIAL_SCAN, defaults={"trust_level": 0.9})

        # Create book titles
        BookTitle.objects.create(book=self.book1, title="Foundation", source=self.data_source, confidence=1.0)
        BookTitle.objects.create(book=self.book2, title="Foundation and Empire", source=self.data_source, confidence=1.0)

        # Create book-author relationships
        BookAuthor.objects.create(book=self.book1, author=self.author, source=self.data_source, confidence=1.0, is_main_author=True)
        BookAuthor.objects.create(book=self.book2, author=self.author, source=self.data_source, confidence=1.0, is_main_author=True)

        # Create book-series relationships
        BookSeries.objects.create(book=self.book1, series=self.series, series_number="1", source=self.data_source, confidence=1.0)
        BookSeries.objects.create(book=self.book2, series=self.series, series_number="2", source=self.data_source, confidence=1.0)

        # Create final metadata for renaming engine
        FinalMetadata.objects.create(book=self.book1, final_title="Foundation", final_author="Isaac Asimov", final_series="Foundation Series", final_series_number="1")
        FinalMetadata.objects.create(book=self.book2, final_title="Foundation and Empire", final_author="Isaac Asimov", final_series="Foundation Series", final_series_number="2")
...
class PatternValidationViewTests(RenamingViewsTestCase):
    ...
    def test_validate_pattern_valid_folder(self):
        ...
        data = {"pattern": "${author_sort}/${series_name}", "type": "folder"}
    ...
    def test_validate_pattern_valid_filename(self):
        ...
        url = reverse("books:renamer_validate_pattern")
        data = {"pattern": "${title}.${ext}", "type": "filename"}
    ...
    def test_validate_pattern_invalid(self):
        ...
        url = reverse("books:renamer_validate_pattern")
        data = {"pattern": "${invalid_token}", "type": "filename"}
    ...
    def test_validate_pattern_with_warnings(self):
        ...
        url = reverse("books:renamer_validate_pattern")
        data = {"pattern": "${title}", "type": "filename"}  # Missing extension
...
class PatternPreviewViewTests(RenamingViewsTestCase):
    ...
    def test_preview_pattern_valid(self):
        ...
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id), str(self.book2.id)]}
    ...
    def test_preview_pattern_single_book(self):
        ...
        url = reverse("books:renamer_preview_pattern")
        data = {"folder_pattern": "Library/${format}/${author_sort}", "filename_pattern": "${series_name} - ${title}.${ext}", "book_ids": [str(self.book1.id)]}
    ...
    def test_preview_pattern_empty_book_list(self):
        ...
        url = reverse("books:renamer_preview_pattern")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": []}
    ...
    def test_preview_pattern_invalid_book_ids(self):
        ...
        url = reverse("books:renamer_preview_pattern")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": ["999", "1000"]}  # Non-existent IDs
    ...
    def test_preview_pattern_warnings(self):
        ...
        url = reverse("books:renamer_preview_pattern")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}", "book_ids": [str(self.book1.id)]}  # Missing extension
...
class BatchRenameExecutionViewTests(RenamingViewsTestCase):
    ...
    def test_execute_batch_rename_dry_run(self, mock_batch_renamer):
        ...
        mock_result = {
            "success": True,
            "dry_run": True,
            "operations": [{"operation_type": "move_file", "source_path": "/test/Foundation.epub", "target_path": "/test/Asimov, Isaac/Foundation.epub", "warnings": []}],
            "summary": {"total_operations": 1, "main_files": 1, "companion_files": 0, "books_affected": 1},
        }
        mock_renamer.execute_operations.return_value = mock_result

        url = reverse("books:renamer_execute_batch")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id)], "dry_run": True, "include_companions": False}
    ...
    def test_execute_batch_rename_actual(self, mock_batch_renamer):
        ...
        mock_result = {
            "success": True,
            "dry_run": False,
            "results": {"successful": 1, "failed": 0, "errors": []},
            "summary": {"total_operations": 1, "main_files": 1, "companion_files": 0, "books_affected": 1},
        }
        mock_renamer.execute_operations.return_value = mock_result

        url = reverse("books:renamer_execute_batch")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id)], "dry_run": False, "include_companions": True}
    ...
    def test_execute_batch_rename_missing_patterns(self):
        ...
        url = reverse("books:renamer_execute_batch")
        data = {"folder_pattern": "", "filename_pattern": "", "book_ids": [str(self.book1.id)], "dry_run": True}
    ...
    def test_execute_batch_rename_no_books(self):
        ...
        url = reverse("books:renamer_execute_batch")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [], "dry_run": True}
    ...
    def test_execute_batch_rename_with_errors(self, mock_batch_renamer):
        ...
        mock_result = {"success": False, "error": "File permission denied", "results": {"successful": 0, "failed": 1, "errors": ["Permission denied: /test/Foundation.epub"]}}
        mock_renamer.execute_operations.return_value = mock_result

        url = reverse("books:renamer_execute_batch")
        data = {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id)], "dry_run": False}
...
class RenamingIntegrationTests(RenamingViewsTestCase):
    ...
    def test_complete_renaming_workflow(self):
        ...
        response = self.client.post(validate_url, {"pattern": "${author_sort}", "type": "folder"})
        self.assertEqual(response.status_code, 200)
        folder_valid = json.loads(response.content)["valid"]
        self.assertTrue(folder_valid)

        # Validate filename pattern
        response = self.client.post(validate_url, {"pattern": "${title}.${ext}", "type": "filename"})
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(preview_url, {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id)]})
        self.assertEqual(response.status_code, 200)
        ...
        with patch("books.views.renaming.BatchRenamer") as mock_batch_renamer:
            ...
            mock_result = {"success": True, "dry_run": True, "operations": [], "summary": {"total_operations": 1, "main_files": 1, "companion_files": 0, "books_affected": 1}}
            mock_renamer.execute_operations.return_value = mock_result

            response = self.client.post(execute_url, {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": [str(self.book1.id)], "dry_run": True})
            self.assertEqual(response.status_code, 200)
    ...
    def test_error_handling_workflow(self):
        ...
        validate_url = reverse("books:renamer_validate_pattern")
        response = self.client.post(validate_url, {"pattern": "${invalid_token}", "type": "filename"})
        self.assertEqual(response.status_code, 200)
        ...
        preview_url = reverse("books:renamer_preview_pattern")
        response = self.client.post(preview_url, {"folder_pattern": "${author_sort}", "filename_pattern": "${title}.${ext}", "book_ids": ["999"]})  # Non-existent ID
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
...
class UserExperienceTests(RenamingViewsTestCase):
    ...
    def test_error_message_quality(self):
        ...
        execute_url = reverse("books:renamer_execute_batch")
        response = self.client.post(execute_url, {"folder_pattern": "", "filename_pattern": "", "book_ids": [], "dry_run": True})
