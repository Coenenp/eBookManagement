...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--folder',
            type=int,
            help='Scan folder ID to test with'
        )
        parser.add_argument(
            '--resume-session',
            type=str,
            help='Resume a specific scan session by ID'
        )
        parser.add_argument(
            '--show-stats',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if options['folder']:
            ...
            self.stdout.write(
                self.style.ERROR('Please specify --folder, --resume-session, --show-stats, or --reset-api-logs')
            )
    ...
    def show_api_statistics(self):
        ...
        self.stdout.write("\nPer-Source Breakdown:")
        for source in DataSource.objects.filter(name__in=[
            DataSource.GOOGLE_BOOKS, DataSource.OPEN_LIBRARY, DataSource.COMICVINE
        ]):
        ...
        # High priority books
        high_priority = BookAPICompleteness.objects.filter(
            scan_priority='high'
        ).count()
