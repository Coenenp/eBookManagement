...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--resume-session", type=str, help="Resume a specific scan session by ID"
        )
        parser.add_argument(
            "--show-stats",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if options["folder"]:
            ...
            self.stdout.write(
                self.style.ERROR(
                    "Please specify --folder, --resume-session, --show-stats, or --reset-api-logs"
                )
            )
    ...
    def test_intelligent_scanning(self, folder_id):
        ...
        try:
            ...
            self.stdout.write(
                f"  Books Needing Retry: {result.get('books_needing_retry', 0)}"
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Scan folder with ID {folder_id} not found")
            )
    ...
    def resume_scan_session(self, session_id):
        ...
        try:
            ...
            if not session.can_resume:
                self.stdout.write(
                    self.style.WARNING("Session cannot be resumed (no pending books)")
                )
                return
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Resume completed! Processed {results['processed']} books"
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Scan session '{session_id}' not found")
            )
        except Exception as e:
    ...
    def show_api_statistics(self):
        ...
        successful_logs = APIAccessLog.objects.filter(
            status=APIAccessLog.SUCCESS
        ).count()
        rate_limited_logs = APIAccessLog.objects.filter(
            status=APIAccessLog.RATE_LIMITED
        ).count()
        ...
        if total_logs > 0:
            self.stdout.write(
                f"  - Successful: {successful_logs} ({successful_logs/total_logs*100:.1f}%)"
            )
            self.stdout.write(
                f"  - Rate Limited: {rate_limited_logs} ({rate_limited_logs/total_logs*100:.1f}%)"
            )
            self.stdout.write(
                f"  - Failed: {failed_logs} ({failed_logs/total_logs*100:.1f}%)"
            )
        ...
        for source in DataSource.objects.filter(
            ...
            if logs.exists():
                success_rate = (
                    logs.filter(status=APIAccessLog.SUCCESS).count()
                    / logs.count()
                    * 100
                )
                self.stdout.write(
                    f"  {source.name}: {logs.count()} attempts, {success_rate:.1f}% success"
                )
        ...
        for session in sessions:
            status = (
                ...
                else "🔄 Active" if session.is_active else "⏸️ Paused"
            )
            resume_status = (
                f" (Resume: {len(session.resume_queue)} pending)"
    ...
    def show_session_stats(self, session):
        ...
        self.stdout.write(
            f"Progress: {session.processed_books}/{session.total_books} ({session.completion_percentage:.1f}%)"
        )
        self.stdout.write(
            f"External data: {session.books_with_external_data} books ({session.external_data_percentage:.1f}%)"
        )
        ...
        if session.api_calls_made:
            ...
            for source, count in session.api_calls_made.items():
                ...
                self.stdout.write(
                    f"  {source}: {count} calls, {success_rate:.1f}% success, {rate_limits} rate limits"
                )

        if session.can_resume:
            self.stdout.write(
                f"Can resume: Yes ({len(session.resume_queue)} books pending)"
            )
        else:
    ...
    def reset_api_logs(self):
        ...
        if (
            input("Are you sure you want to reset all API logs? (yes/no): ").lower()
            == "yes"
        ):
