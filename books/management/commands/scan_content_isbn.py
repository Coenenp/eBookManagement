...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["filename"]:
            queryset = queryset.filter(files__file_path__icontains=options["filename"])

        if options["file_type"]:
            ext = f".{options['file_type']}"
            queryset = queryset.filter(files__file_path__iendswith=ext)
        ...
        if not options["force"]:
            ...
            books_with_content_isbn = BookMetadata.objects.filter(field_name="isbn", source__name=DataSource.CONTENT_SCAN).values_list("book_id", flat=True)
        ...
        if options["book_id"]:
            try:
                book = queryset.get()
                self.stdout.write(f"Scanning book: {book.primary_file.filename if book.primary_file else 'unknown'}")
                save_content_isbns(book, page_limit=options["pages"])

                # Check results
                content_isbns = BookMetadata.objects.filter(book=book, field_name="isbn", source__name=DataSource.CONTENT_SCAN).count()
